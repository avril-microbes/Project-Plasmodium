...
plot_basis_spline <- function(par){
  ...
  mod_bs.p <- mod_bs %>% 
    mutate(
      ...
      first_spline = ((`1`*par[2])+par[1])/max,
       second_spline = ((`2`*par[3])+par[1])/max,
       third_spline = ((`3`*par[4])+par[1])/max,
      fit = cr_fit/max
           )
  ...
  ggplot(mod_bs.p) +
    ...
    geom_line(aes(x = cue_range, y = first_spline, color = "Spline 1"), linetype = "dashed") +
    geom_line(aes(x = cue_range, y = second_spline, color = "Spline 2"), linetype = "dashed") +
    geom_line(aes(x = cue_range, y = third_spline, color = "Spline 3"), linetype = "dashed") +
    theme_bw() +
    labs(x = "Cue range", y = "Transmission investment", color = "Legend") +
    theme(
...
mc_all_fitness.df_p <- mc_all_fitness.df %>% 
  ...
  filter(!short_label %in% c("R & I log","R & I+Ig log"))
...
dual_dc.df <- dual_cue_dyn_30.df %>% 
  mutate(label_alt = gsub("I\\+Ig", "Total I", paste(label, "&" , label_b))) %>% ## get new label
...
si_dc.plt <- ggplot() +
  ...
  scale_color_manual(values=c( "#4575b4", "#fdcb44", "#91bfdb", "#fc8d59", "black"), name = "Cues")  +
...
dual_dc.plt <- ggplot() +
  ...
  scale_color_manual(values=c( "#4575b4", "#fdcb44", "#91bfdb", "#fc8d59", "black"))  +
    labs(fill = "Cues", x = "Total iRBC", y = "RBC", color = "Cues") +
...
# single cue
## top 4 cues
si_opt_area.df_4 <- si_opt_area.df %>% filter(id %in% top_4_single_cue$id)
si_opt_area.df_4$short_label <- factor(si_opt_area.df_4$short_label, c("Ig log", "I+Ig log", "G log", "I log"))

si_corr_area.plt1 <- ggplot() +
  geom_point(data = si_opt_area.df_4, aes(x = area, y = fitness_norm, color = short_label, shape = short_label), size = 3, alpha = 0.7) +
       scale_color_manual(values=c( "#4575b4", "#fdcb44", "#91bfdb", "#fc8d59", "black"), name = "Cues") 
...
si_corr_area.plt3 <- si_corr_area.plt2+
  geom_point(data = si_opt_area.df%>% filter(!id %in% top_4_single_cue$id), aes(x = area, y = fitness_norm), color = "grey", size = 3) +
  geom_point(aes(x = time_high.area, y = max(time_high.dyn %>% filter(variable == "tau_cum" & time == 20) %>% pull(value))/9.883602, shape = "Time"), size = 3, color = "black") +
...
```{r}
# dual cue
dual_cue_f_final_area.df_4 <- dual_cue_f_final_area.df %>% filter(label_alt %in% top_4_dual_cue$label_alt)

dual_cue_f_final_area.df_4$label_alt <- factor(dual_cue_f_final_area.df_4$label_alt, c("R log & I log", "R & I log", "R & Total I log", "R log & Total I log"))


dual_corr_area.plt1 <- ggplot() +
  geom_point(data = dual_cue_f_final_area.df_4, aes(x = area, y = fitness_norm, color = label_alt, shape = label_alt), size = 3, alpha = 0.7) +
       scale_color_manual(values=c( "#4575b4", "#fdcb44", "#91bfdb", "#fc8d59", "black"), name = "Cues") 

dual_corr_area.plt2 <- dual_corr_area.plt1 + ggnewscale::new_scale_color()

dual_corr_area.plt3 <- dual_corr_area.plt2 +
  geom_point(data = dual_cue_f_final_area.df%>% filter(!label_alt %in% top_4_dual_cue$label_alt), aes(x = area, y = fitness_norm), size = 3, alpha = 0.7, color = "grey") +
  geom_point(aes(x = time_high.area, y = max(time_high.dyn %>% filter(variable == "tau_cum" & time == 20) %>% pull(value))/9.883602,  shape = "Time"), size = 3, color = "black") +
