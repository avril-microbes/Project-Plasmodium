...
dual_cue_f_glb.df <- read.csv(here("code_repository/data/dual_cue_fitness_global.csv")) ## optimized dual cue strategy using DEoptim + L-BFG-B
dual_cue_f_init.df <- read.csv(here("code_repository/data/dual_cue_fitness_init.csv")) ## optimized dual cue strategy using optimized initial starting point + L-BFGS-B
...
```
#=================================#
Illustrating spline optimization
#=================================#
# function to extract basis spline and plot
```{r}
plot_basis_spline <- function(par){
  # build dummy basis spline model
  dummy_cr.mod <- lm(rep(0, 101)  ~ splines2::bSpline(x = seq(0,100,1), degree = 3))
  
  # assign parameters
  dummy_cr.mod$coefficients <- par
  
  # get fit
  cr_fit <- predict(dummy_cr.mod, newdata = data.frame(seq(0,100,1)))
  
  # get basis spline database
  mod_bs <- dummy_cr.mod$model$`splines2::bSpline(x = seq(0, 100, 1), degree = 3)` %>% as.data.frame()
  
  # combine database
  max <- max(mod_bs$`1`,mod_bs$`2`,mod_bs$`3`, cr_fit,na.rm = T)
  mod_bs.p <- rbind(
    mod_bs %>% mutate(cue_range = seq(0,100,1), spline_value = ((`1`*par[2])+par[1])/max,fit = cr_fit/max, label = c("Column 1")),
    mod_bs %>% mutate(cue_range = seq(0,100,1), spline_value = ((`2`*par[3])+par[1])/max,fit = cr_fit/max, label = c("Column 2")),
    mod_bs %>% mutate(cue_range = seq(0,100,1), spline_value = ((`3`*par[4])+par[1])/max,fit = cr_fit/max, label = c("Column 3"))

    )
   # plot
  ggplot(mod_bs.p) + 
    geom_line(aes(x = cue_range, y = fit), color = "Black") +
    geom_line(aes(x = cue_range, y = spline_value, color = label, linetype = label)) +
    theme_bw() +
    labs(x = "Cue range", y = "Transmission investment", linetype = "Basis matrix", color = "Basis matrix") +
...
# plot for multiple parameters
```{r}
bspline.pl1 <- plot_basis_spline(c(0,0.5,0.5,0.5))
bspline.pl2 <- plot_basis_spline(c(0, 2, 6, 5))
bspline.pl3 <- plot_basis_spline(c(0 , 0, 20 , 10))

# plot together
ggarrange(bspline.pl1, bspline.pl2, bspline.pl3, common.legend = T, ncol = 3, legend = "right")
ggsave(units = "px", dpi = 300, width = 2250, height = 600, filename = here("code_repository/figures/reaction_norm_opt.tiff"), bg = "white")
```
...
fitness_rank.plt <- ggplot(si_opt.df, aes(x = fitness_norm, y = fct_reorder(long_label, fitness_norm))) +
  geom_bar(stat = "identity", fill = "black") +
  labs(x = "Normalized fitness", y = "Cue") +
  scale_x_continuous(limits = c(0.7, 1), oob = rescale_none) +
  theme_classic() +
...
si_rn.df_p <- si_rn.df %>% 
  ...
  mutate(log2 = ifelse(log == "log", "log", "raw"))
...
fitness_rn.plt <- ggplot() +
  geom_line(data = si_rn.df_p, aes(x = log10(cue_range), y = cr, color = log2, linetype = log2)) +
   geom_rug(data = si_rug_cue.df_none%>% filter(value>1), aes(x = log10(value)), color = "black", sides = "t", length = unit(0.1, "npc")) +
  geom_rug(data = si_rug_cue.df_log%>% filter(value>1), aes(x = log10(value)), color = "#D41159", sides = "b", length = unit(0.1, "npc")) +
  facet_wrap(~cue_label, scales = "free_x", ncol = 1) +
  labs(x = "log10(Cue range)", y = "Transmission investment", color = "Cue status", shape = "Cue status", linetype = "Cue status") +
  ylim(-0.2, 1) +
  scale_color_manual(values=c("#D41159", "black")) +
...
dual_cue_f_final.df <- dual_cue_f_init.df %>% 
  select(id, id_b, label, label_b, fitness_init = fitness, 
         par1_init = par1, par2_init = par2, par3_init = par3, par4_init = par4, par5_init = par5, par6_init = par6,
         par7_init = par7, par8_init = par8, par9_init = par9) %>% 
  left_join(
  select(dual_cue_f_glb.df, id, id_b, label, label_b, fitness_glb = fitness, 
  ...
  ) %>% ## rename columns and join global and local optimization fitness dataframe
  group_by(id, id_b) %>% 
  mutate(
    fitness = max(fitness_glb, fitness_lc, fitness_init),
    algo = case_when(
      fitness_glb == fitness ~ "glb",
      fitness_lc == fitness ~ "lc",
      fitness_init == fitness ~ "init"
    ),
    par1 = case_when(algo == "glb" ~ par1_glb, algo == "lc" ~ par1_lc, algo == "init" ~ par1_init),
    par2 = case_when(algo == "glb" ~ par2_glb, algo == "lc" ~ par2_lc, algo == "init" ~ par2_init),
    par3 = case_when(algo == "glb" ~ par3_glb, algo == "lc" ~ par3_lc, algo == "init" ~ par3_init),
    par4 = case_when(algo == "glb" ~ par4_glb, algo == "lc" ~ par4_lc, algo == "init" ~ par4_init),
    par5 = case_when(algo == "glb" ~ par5_glb, algo == "lc" ~ par5_lc, algo == "init" ~ par5_init),
    par6 = case_when(algo == "glb" ~ par6_glb, algo == "lc" ~ par6_lc, algo == "init" ~ par6_init),
    par7 = case_when(algo == "glb" ~ par7_glb, algo == "lc" ~ par7_lc, algo == "init" ~ par7_init),
    par8 = case_when(algo == "glb" ~ par8_glb, algo == "lc" ~ par8_lc, algo == "init" ~ par8_init),
    par9 = case_when(algo == "glb" ~ par9_glb, algo == "lc" ~ par9_lc, algo == "init" ~ par9_init)
  ) ## the columns are assigned based on which strategy produced the highest fitness
```

# rerunning dual cue simulations to match single cue time step
```{r}
# function to rerun dual cue simulation
get_dual_fitness <- function(df){
  ## processing model input
  ...
  # append id
  res <- cbind.data.frame(id = id, id_b = id_b, fitness_0.001 = fitness)
...
dual_cue_f_final.df2 <- dual_cue_f_final.df %>% 
  left_join(select(dual_cue.df3, id, id_b, cue, cue_b, log, log_b, low, low_b, high, high_b), by = c("id", "id_b"))

# split and run
dual_cue_f_final_df2.ls <-split(dual_cue_f_final.df2 , seq(nrow(dual_cue_f_final.df2)))
dual_cue_rerun_fitness <- mclapply(dual_cue_f_final_df2.ls, get_dual_fitness, mc.cores = 8)

# concatenate
dual_cue_rerun_fitness.df <- do.call(rbind, dual_cue_rerun_fitness)

# join back
dual_cue_f_final.df3 <- left_join(dual_cue_f_final.df, dual_cue_rerun_fitness.df, by = c("id", "id_b"))

## write csv
write.csv(dual_cue_f_final.df3, here("code_repository/data/dual_cue_fitness_final.csv"))
```
...
dual_si_fitness.df <- dual_cue_f_final.df %>% 
  select(id, id_b, label, label_b, fitness_dual = fitness_0.001) %>% 
  ...
  mutate(fitness_si_final = ifelse(fitness_si > fitness_si_b, fitness_si, fitness_si_b)/9.883602,
         ...
         fitness_dual_norm = fitness_dual/9.883602) 

# write csv
write.csv(dual_si_fitness.df, here("code_repository/data/dual_cue_si_fitness_final.csv"))

# filter to keep only cues with only 2 logs
dual_si_fitness.df_f <-dual_si_fitness.df %>% filter(str_detect(id, "log") & str_detect(id_b, "log")) ## keep only cues that have two logs
```

## plot fitness rank
```{r}
dual_fitness.pl <- ggplot(dual_si_fitness.df_f) +
...
Rlog_Ilog.cr <- chabaudi_si_clean(
  parameters_cr = c(6.875066649,	1.746121145,	0.678992033,	25.27235333,	-29.39793239,	-15.10449299,	43.5303092,	-7.664658261,	11.64534955),
  immunity = "tsukushi",
  parameters = parameters_tsukushi,
...
# process 
I_high.cr_p <- I_high.cr %>% filter(variable == "cr") %>% mutate(label_new = "I log (df=9)") %>% select(-variable)

R_high.cr_p <- R_high.cr %>% filter(variable == "cr") %>% mutate(label_new = "R log (df=9)") %>% select(-variable)
...
Rlog_Ilog.hm <- par_to_hm_te(par = c(6.875066649,	1.746121145,	0.678992033,	25.27235333,	-29.39793239,	-15.10449299,	43.5303092,	-7.664658261,	11.64534955),
...
ggsave(here("code_repository/figures/dual_main_intermediate.tiff"), units = "px", width = 2250, height = 1400, scale = 1.1, dpi=300,  bg = "white")
```

#=================================#
# Dual cue conversion rate supplementary figure (cr dynamics)
#=================================#
...
dual_cue_cr.df <- dual_cue_dyn.df %>% 
  filter(variable == "cr" & str_detect(id, "log") & str_detect(id_b, "log")) %>% 
  left_join(select(dual_cue_f_final.df, id, id_b, fitness), by = c("id", "id_b")) %>% 
  mutate(label_comb = gsub("I\\+Ig", "Total I", paste(label, "&", label_b)))
...
ggsave(here("code_repository/figures/dual_cue_cr.tiff"), units = "px", width = 2000, height = 1000, dpi=300,  bg = "white")
```

#=================================#
# Cumulative transmission plot of dual and single cue models
#=================================#
# get cumulative tau
```{r}
# filter and add names
...
# bind together
dual_selected_taucum.df <- rbind(I_high.taucum, R_high.taucum, time_high.taucum, Rlog_Ilog.taucum)

# write
write_parquet(dual_selected_taucum.df, here("code_repository/data/dual_selected_taucum.parquet"))
```
...
#=================================#
# dynamics of time-based parasite (supplementary figure)
#=================================#
# process data
```{r}
# keep only dynamics of parasite that senses time and variables that we are interested in
si_dyn_time.df <- si_dyn.df %>% filter(id == "time" & !variable %in% c("cr_t", "tau", "tau_cum", "ID", "N", "W", "M", "Mg")) %>% 
  mutate(variable_label = case_when(
    variable == "cr" ~ "Transmission investment (c)",
    variable == "G" ~ "log10(Gametocyte)",
    variable == "I" ~ "log10(Asexual iRBC)",
    variable == "Ig" ~ "log10(Sexual iRBC)",
    variable == "R" ~ "log10(RBC)"
  ),
  values_logged = if_else(variable == "cr", value, log10(value))) %>% 
  group_by(variable) %>% 
  arrange(time, .by_group = T) %>% 
  filter(row_number() %% 10 == 0 & values_logged>=0)

# reorder variable
si_dyn_time.df$variable_label <- factor(si_dyn_time.df$variable_label, c("Transmission investment (c)",
                                                                       "log10(RBC)",
                                                                       "log10(Asexual iRBC)","log10(Sexual iRBC)","log10(Gametocyte)"))
```

# plot
```{r}
# plot only cr
si_dyn_time_cr.pl <- ggplot(si_dyn_time.df %>% filter(variable == "cr") , aes(x = time, y = values_logged)) +
  labs(x = "Time (days)", y = "Transmission\ninvestment") +
  geom_rect(aes(xmin = 0,xmax=2,ymin=-Inf,  ymax = Inf),fill = "#AE73F4" , alpha = 0.2) +
  geom_rect(aes(xmin = 15,xmax=20,ymin=-Inf,  ymax = Inf), fill = "#ED8080" , alpha = 0.2) +
    geom_line() +
  theme_bw() +
...
si_dyn_time_rest.pl <- ggplot(si_dyn_time.df %>% filter(variable != "cr"), aes(x = time, y = values_logged)) +
...
# plot together
## get a narrower version of the cr plot
ggarrange(si_dyn_time_cr.pl, si_dyn_time_rest.pl, ncol = 2, nrow =1, align = "h", widths = c(1.75,4))
ggsave(here("code_repository/figures/dual_cue_preamble.tiff"), units = "px", width = 2250, height = 400, dpi=300,  bg = "white", scale = 1.2)
...
# process data
```{r}
time_high.dyn_wide <- time_high.dyn %>% 
  filter(time<=20) %>% 
  mutate(value = ifelse(variable != "cr", value+1, value)) %>% 
  pivot_wider(id = "time", names_from = variable, values_from = value)
```
...
ggarrange(
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(I), y = log10(R), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(I), y = log10(R)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
  labs(x = "Asexual iRBC log", y = "RBC log", color = "Transmission investment") +
  xlim(0,NA) +
  theme_bw() +
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(I+Ig), y = log10(R), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(I+Ig), y = log10(R)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
    xlim(0,NA) +
  labs(x = "Total iRBC log", y = "RBC log", color = "Transmission investment") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(I), y = log10(G), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(I), y = log10(G)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
    xlim(0,NA) +
    ylim(0,NA) +
  labs(x = "Asexual iRBC log", y = "Gametocyte log", color = "Transmission investment") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(Ig), y = log10(G), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(Ig), y = log10(G)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
    ylim(0,NA) +
    xlim(-1,NA) +
  labs(x = "Sexual iRBC log", y = "Gametocyte log", color = "Transmission investment") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(Ig), y = log10(R), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(Ig), y = log10(R)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
    xlim(-1,NA) +
  labs(x = "Sexual iRBC log", y = "RBC log", color = "Transmission investment") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(Ig), y = log10(I), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(Ig), y = log10(I)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
    xlim(-1,NA) +
    ylim(0,NA) +
  labs(x = "Sexual iRBC log", y = "Asexual iRBC log", color = "Transmission investment") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(I+Ig), y = log10(Ig), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(I+Ig), y = log10(Ig)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
    xlim(0,NA) +
    ylim(-1,NA) +
  labs(x = "Total iRBC log", y = "Sexual iRBC log", color = "Transmission investment") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(R), y = log10(G), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(R), y = log10(G)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
    ylim(0,NA) +
  labs(x = "RBC log", y = "Gametocyte log", color = "Transmission investment") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(I+Ig), y = log10(I), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(I+Ig), y = log10(I)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
  labs(x = "Total iRBC log", y = "Asexual iRBC log", color = "Transmission investment") +
  theme_bw() +
    xlim(0,NA) +
    ylim(0,NA) +
  theme(panel.grid.major = element_blank(),
  ...
  ggplot() +
  geom_path(data = time_high.dyn_wide, aes(x = log10(I+Ig), y = log10(G), color = cr), size = 1.5, alpha = 1) +
  geom_point(data = time_high.dyn_wide %>% filter(time %% 1 == 0), 
                      aes(x = log10(I+Ig), y = log10(G)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
    xlim(0,NA) +
    ylim(0,NA) +
  labs(x = "Total iRBC log", y = "Gametocyte log", color = "Transmission investment") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
  common.legend = T, align = "hv", ncol =3, nrow = 4
)

ggsave(here("code_repository/figures/sim_disease-curve.tiff"), units = "px", width = 2250, height = 2000, scale = 1, dpi=300,  bg = "white")
...
cue_cue_rn_pl.ls <- lapply(dual_cue_rn.ls,
       function(x){
         ...
         ggplot() +
           ...
           #scale_x_continuous(labels = label_scientific(digits = 2),breaks= breaks_pretty(3)) +
           #scale_y_continuous(labels = label_scientific(digits = 2),breaks= breaks_pretty(2)) 
           scale_x_continuous(breaks= breaks_pretty(3)) +
           scale_y_continuous(breaks= breaks_pretty(3)) 
       })

## arrange together
cue_cue_rn.pl <- ggarrange(plotlist = cue_cue_rn_pl.ls, ncol = 4, nrow = 10, common.legend = T, align = "hv") 
ggsave(here("code_repository/figures/sim_disease-curve.tiff"), units = "px", width = 2250, height = 2625, scale = 1.5, dpi=300,  bg = "white")
```
...
# process data
```{r}
# filter dataset and make wide
time_high_sel.dyn <- time_high.dyn %>% 
  filter(variable %in% c("R", "I", "G","cr") & time<=20) %>% 
  pivot_wider(id = "time", names_from = variable, values_from = value) 
```
...
ggarrange(
  # RBC log vs I log
  ggplot() +
  geom_path(data = time_high_sel.dyn, aes(x = log10(I), y = log10(R), color = cr), size = 1.5) +
  geom_point(data = time_high_sel.dyn %>% filter(time %% 1 == 0), 
                      aes(x = log10(I), y = log10(R)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
  labs(x = "Asexual iRBC log", y = "RBC log", color = "Transmission investment") +
  theme_bw() +
  theme(
   ...
   exp_xlogylog.pl_ls[[1]] + coord_flip() + 
  theme_bw() +
  theme( legend.position = "none",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ...
  # RBC log vs I 
ggplot() +
  geom_path(data = time_high_sel.dyn, aes(x = I, y = log10(R), color = cr), size = 1.5) +
  geom_point(data = time_high_sel.dyn %>% filter(time %% 1 == 0), 
                      aes(x = I, y = log10(R)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
  labs(x = "Asexual iRBC", y = "RBC log", color = "Transmission investment") +
  theme_bw() +
  theme(legend.position = "none",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ...
  exp_xlogy.pl_ls[[1]] + coord_flip() + theme_bw() +
  theme(legend.position = "none",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
 ...
 # RBC log vs G log
  ggplot() +
  geom_path(data = time_high_sel.dyn, aes(x = log10(G), y = log10(R), color = cr), size = 1.5) +
  geom_point(data = time_high_sel.dyn %>% filter(time %% 1 == 0), 
                      aes(x = log10(G), y = log10(R)), size = 1.5, shape = 1) +
  scale_color_viridis_c(limits = c(0, 1.01)) +
  xlim(0, NA) +
  labs(x = "Gametocyte log", y = "RBC log", color = "Transmission investment") +
  theme_bw() +
  theme(
    legend.position = "none",
        panel.grid.major = element_blank(),
    ...
    exp_xlogylog.pl_ls[[2]] + coord_flip() + ylim(0,NA) + theme_bw() +
  theme(legend.position = "none",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()
  ),
...
ggsave(here("code_repository/figures/sim_exp_disease_curve_main.tiff"), units = "px", width = 1500, height = 1500, scale = 1.25, dpi=300,  bg = "white")
...
ggsave(here("code_repository/figures/sim_exp_disease_curve_legend.tiff"), units = "px", width = 1500, height = 500, scale = 1.25, dpi=300,  bg = "white")
...
## we are expecting 11*5010 = 55110 data files for single cue and 10*1000=10000 files for dc
length(mc_all_sc.ls)
length(mc_all_dc.ls)
...
mc_all_fitness_dc.df %>% 
  distinct(cue, cue_b, log, log_b, iter) %>% 
...
mc_all_fitness.df %>% filter(iter == 1) %>% 
  ...
  mutate(diff = fitness - fitness_20)
...
mc_all_fitness.df_p <- mc_all_fitness.df %>% 
  filter(iter %in% seq(1,1000,1) & id != "time") %>% 
  mutate(fitness_norm = fitness/9.883602,
         cue_condition = ifelse(str_detect(id, "-"), "Dual cue", "Single cue"),
         short_label = ifelse(str_detect(id, "-"), gsub("-"," & ",gsub("_", " ", id)), short_label), ## if dual cue, derive short label from id!
         short_label = gsub("I\\+Ig", "Total I", short_label)) ## substitute I+Ig with Total I 
...
## get mean and mode in a separate df
  ...
  summarize(geom_mean = exp(mean(log(fitness_norm))),
            sd = sd(fitness_norm)) %>% 
  left_join(select(mc_all_fitness_ref.df, short_label, fitness, cue_condition), by = "short_label")
```

# plot
```{r}
mc_all_fitness.pl <- ggplot() +
  ggrepel::geom_label_repel(data = mc_all_fitness_sum.df, aes(x = fitness/9.883602, y = geom_mean, label = short_label, colour = factor(cue_condition)),box.padding
=0.1, size = 3, label.padding = 0.1, force_pull = 0.2, force = 30) +
  geom_point(data = mc_all_fitness_sum.df, aes(x = fitness/9.883602, y = geom_mean, shape = cue_condition, color = cue_condition), size = 2) +
  #geom_errorbar(data = mc_all_fitness_sum.df, aes(x = fitness, y = geom_mean, ymin = geom_mean-sd, ymax = geom_mean+sd, color = cue_condition)) +
  labs(x = "Relative deterministic model fitness", y = "Relative semi-stochastic model fitness (geometric mean)", color = "Legend", shape = "Legend", label = "") +
...
#------------Correlation of cue variation on fitness decline--------------#
# import data
```{r}
# I log
## to get the right file names, I limited the length of the file name to filter out the dual cue names and kept only the first 1000 interations
mc_ilog_dyn.df_file <- Filter(function(x) str_length(basename(x)) <= 20 & as.numeric(str_extract(x, "(?<=_)[0-9]+(?=\\.parquet$)"))<=1000,
       list.files(here("code_repository/data/mc_all_dyn"), pattern = "I_log10_[0-9]*.parquet", full.names = T))
## read and combine
mc_ilog_dyn.df <- do.call(rbind, mclapply(mc_ilog_dyn.df_file, 
                                          function(x){
                                              data <- read_parquet(x) %>% mutate(iter = str_extract(x, "(?<=_)[0-9]+(?=\\.parquet$)"))
...
## R log and I log
mc_rlog_ilog_dyn.df_file <- Filter(function(x) as.numeric(str_extract(x, "(?<=_)[0-9]+(?=\\.parquet$)"))<=1000,
       list.files(here("code_repository/data/mc_all_dyn"), pattern = "R_log10-I_log10", full.names = T))
## read and combine
mc_rlog_ilog_dyn.df <- do.call(rbind, mclapply(mc_rlog_ilog_dyn.df_file, 
                                          function(x){
                                              data <- read_parquet(x) %>% mutate(iter = str_extract(x, "(?<=_)[0-9]+(?=\\.parquet$)"))
...
# filter data to keep only cues
```{r}
# Here, I am only keeping the relevant cues and conversion rate
mc_ilog_dyn.df_cue <- mc_ilog_dyn.df %>% 
  filter(variable %in% c("I", "R", "cr"))%>% 
  mutate(value_logged = ifelse(variable == "cr", value, log10(value))) %>% 
  pivot_wider(id_cols = c(time, iter), names_from = variable, values_from = value_logged)

mc_rlog_ilog_dyn.df_cue <- mc_rlog_ilog_dyn.df %>% 
  filter(variable %in% c("I", "R", "cr"))%>% 
  mutate(value_logged = ifelse(variable == "cr", value, log10(value))) %>% 
  pivot_wider(id_cols = c(time, iter), names_from = variable, values_from = value_logged) %>% 
  group_by(iter) %>% 
...
# plot variation of cue over time
```{r}
si_rn.df_p_Ilogsi_rn.df_p %>% filter(id == "I_log") %>% rbind(data.frame(cue_range = 10^7, cr = 0.3049818, id = "I_log", cue= "I", log = "log", min = 0, max = 113580.5, cue_label = "Asexual iRBC", log2 = "log"))

si_rn.df %>% filter(id == "I_log") %>% arrange(desc(cue_range))
```

```{r}
# I log
## cue over time
mc_ilog_cue.pl <- ggplot() +
  geom_tile(data = si_rn.df %>% filter(id == "I_log"), aes(y = log10(cue_range), x= 10, width = 20, fill = cr)) +
  geom_line(data = mc_ilog_dyn.df_cue, aes(x = time, y = I, group = iter), alpha = 0.05, color = "#D9615F") +
 # geom_path(data = mc_ilog_dyn.df_cue %>% filter(iter == 1), aes(x = time, y = I), alpha = 1, size = 2, color = orange,arrow = arrow()) +
  geom_path(data = mc_ilog_dyn.df_cue %>% filter(iter == 1), aes(x = time, y = I), alpha = 1, size = 2, color = "white",arrow = arrow()) +
  labs(x = "Time (days)", y = "Asexual iRBC log", fill = "Transmission\ninvestment") +
  #scale_fill_gradientn(colours = grey(seq(0,1,l=20)), limit = c(0,1)) +
...
# cr over time
mc_ilog_cr.pl <- ggplot() +
  geom_line(data = mc_ilog_dyn.df_cue, aes(x = time, y = cr, group = iter), alpha = 0.1, color = "grey") +
  geom_line(data = mc_ilog_dyn.df_cue %>% filter(iter == 1), aes(x = time, y = cr), alpha =1,size = 2, color = orange) +
  labs(x = "Time (days)", y = "Transmission investment", color = "Time (days)") +
...
# R log and I log
# cue over time
mc_ilog_cue.pl2 <- ggplot() +
  geom_raster(data = Rlog_Ilog.hm, aes(x = cue_range_b, y = cue_range, fill = cr)) +
  geom_path(data = mc_rlog_ilog_dyn.df_cue, aes(y = R, x = I, group = iter), alpha = 0.2, color = "#D9615F") +
  #geom_path(data =mc_rlog_ilog_dyn.df_cue %>% filter(iter == 1), aes(y = R, x = I), alpha = 1, size = 2, color = blue, arrow = arrow()) +
  geom_path(data =mc_rlog_ilog_dyn.df_cue %>% filter(iter == 1), aes(y = R, x = I), alpha = 1, size = 2, color = "white", arrow = arrow()) +
  labs(x = "Asexual iRBC log", y = "RBC log", fill = "Transmission\ninvestment") +
  #scale_fill_gradientn(colours = grey(seq(0,1,l=20)), limit = c(0,1)) +
  scale_fill_viridis_c(limit = c(0,1)) +
  xlim(1.75, 6.3) +
  ylim(6.65, 7) +
...
# cr over time
mc_rlog_ilog_cr.pl <- ggplot() +
  geom_line(data = mc_rlog_ilog_dyn.df_cue, aes(x = time, y = cr, group = iter), alpha = 0.1, color = "grey") +
  geom_line(data = mc_rlog_ilog_dyn.df_cue %>% filter(iter == 1), aes(x = time, y = cr, group = iter), alpha = 1, size = 2, color = blue) +
  labs(x = "Time (days)", y = "Transmission investment", color = "Time (days)") +
  #scale_color_viridis_c(option = "A") +
  ylim(0,1) +
  theme_bw() +
...
# plot together
mc_cue.pl <- ggarrange(mc_ilog_cue.pl, mc_ilog_cue.pl2, mc_ilog_cr.pl,  mc_rlog_ilog_cr.pl, common.legend = T, ncol = 2, nrow = 2, align = "hv")
```
...
## read and combine
mc_single_fitness.df <- do.call(rbind, mclapply(mc_single.ls, 
                                                function(x){
                                                  df <- read.csv(x)
                                                  names(df) <- c("X","iter",	"rho",	"burst",	"iota_N1", "iota_N2",	"phi_N1",	"phi_N2",	"fitness_rho",	"fitness_beta",	"fitness_psin",	"fitness_psiw", "fitness_phin",	"fitness_phiw",	"id")
                                                  return(df)
                                                  }, 
...
mc_single_all_fitness.df <- mc_single_fitness.df %>% 
  left_join(select(mc_all_fitness.df_p, fitness_all = fitness,id, iter, short_label), by = c("id", "iter")) %>% 
...
mc_single_fitness.long <- mc_single_all_fitness.df %>% 
  ...
  left_join(select(mc_all_fitness_ref.df, fitness_ref = fitness, id),
            by = c("id")) %>% ## add in the deterministic fitness values
  select(id, short_label,fitness_rho, fitness_beta, fitness_psin, fitness_psiw, fitness_phin, fitness_phiw, iter, fitness_all, fitness_ref) %>% # keep only fitness and associated labels
  tidyr::pivot_longer(-c("id", "short_label","fitness_all", "iter", "fitness_ref")) %>% ## make long
...
mc_single_fitness.long_p <- mc_single_fitness.long %>% 
  ...
  left_join(ez_label %>% select(-short_label), by = c("id"))
...
## rank by descending performance in deterministic fitness
mc_single_fitness.long_p_f$short_label <- factor(mc_single_fitness.long_p_f$short_label,c(mc_all_fitness_ref.df %>% arrange(desc(fitness_norm)) %>% select(short_label))$short_label)
...
mc_partition.pl <- ggplot() +
  geom_violin(data = mc_single_fitness.long_p_f %>% filter(str_detect(short_label, "log")), aes(x = rel_diff_single, y = short_label,
                                                 ), fill = "grey", color = "grey") + ## 89% credible interval
  geom_point(data = mc_single_fitness.sum %>% filter(str_detect(short_label, "log")), aes(x = median, y = short_label)) +
...
```{r}
# v1
ggarrange(mc_all_fitness.pl, mc_partition.pl, align = "h", widths = c(1.5, 2), labels = c("A", "B"))
ggsave(units = "px", dpi = 300, width = 2250, height = 1500, filename = here("code_repository/figures/mc_fitness_partition.tiff"), bg = "white", scale = 1.3)

# v2
ggarrange(mc_all_fitness.pl, mc_cue.pl, align = "h", widths = c(1, 1.5), labels = c("A", "B"))
ggsave(units = "px", dpi = 300, width = 2250, height = 1250, filename = here("code_repository/figures/mc_fitness_partition.tiff"), bg = "white", scale = 1.3)

ggsave(units = "px", dpi = 300, width = 2250, height = 1250, filename = here("code_repository/figures/mc_fitness_partition_v2.tiff"), bg = "white", scale = 1.3)
```
...
posterior.long <- posterior.df %>% 
  filter(iter %in% seq(0,1000,1)) %>% 
  tidyr::pivot_longer(-iter, names_to = "parameter") %>% 
...
## process data for plotting
```{r}
## calculate geometric mean fitness for each parameter variation set
mc_single_fitness.sum2 <- mc_single_fitness.long %>% 
  filter(str_detect(id, "log")) %>% 
  mutate(fitness_norm = value/9.883602,
         fitness_ref_norm = fitness_ref/9.883602,
         cue_type = ifelse(str_detect(short_label, "&"), "Dual cue", "Single cue"),
  ...
  group_by(short_label, parameter_label, fitness_ref_norm, cue_type) %>% 
  summarise(geom_mean = exp(mean(log(fitness_norm))))
```
# plot
```{r}
ggplot() +
  ggrepel::geom_label_repel(data = mc_single_fitness.sum2 %>% filter(short_label %in% c("R log & I log", "R log & Total I log")), aes(x = fitness_ref_norm, y = geom_mean, color = cue_type, label = short_label), box.padding
=0.1, size = 3, label.padding = 0.1, force_pull = 0.2, force = 30, nudge_x = -0.07, nudge_y = -0.02, show.legend = F) +
    geom_point(data = mc_single_fitness.sum2%>% filter(short_label %in% c("R log & I log", "R log & Total I log")) , aes(x = fitness_ref_norm , y = geom_mean), size = 3.5, color = "black", shape = 17) +
  geom_point(data = mc_single_fitness.sum2, aes(x = fitness_ref_norm, y = geom_mean, color = cue_type, shape = cue_type), size = 2.5) +
  labs(x = "Relative deterministic model fitness", y = "Relative semi-stochastic model fitness (geometric mean)", color = "Legend", shape = "Legend", label = "") +
  scale_color_manual(values = c(blue, orange)) +
    scale_shape_manual(values = c(17, 16)) +
...
ggsave(units = "px", dpi = 300, width = 2500, height = 1500, filename = here("code_repository/figures/mc_single_fitness.tiff"), bg = "white", scale = 1)
...
mc_single_dyn_cr_join.df_p <- read_parquet(here("code_repository/data/mc_single_dyn_cr_join.parquet"))
...
mc_single_dyn_cr.sum <- mc_single_dyn_cr_join.df_p %>% 
  ...
  summarise(mean_cr_diff = mean(diff),
            median_cr_diff = median(diff),
            sd_cr_diff = sd(diff),
            mad_cr_diff= mad(diff))
...
#====================================#
#--------- Simulate dual cue data ---------#
# rerunning dual cue simulations to match single cue time step
```{r}
# function to rerun dual cue simulation
get_dual_dyn <- function(df){
  ## processing model input
  par <- c(df$par1, df$par2, df$par3, df$par4, df$par5,df$par6,df$par7, df$par8, df$par9)## parameters
  cue <- df$cue ## cue choice
  cue_b <- df$cue_b
...
# split and run
dual_cue_dyn <- mclapply(dual_cue_f_final_df2.ls, get_dual_dyn, mc.cores = 8)

# concatenate
dual_cue_dyn.df <- do.call(rbind, dual_cue_dyn)

## write csv
write_parquet(dual_cue_dyn.df, here("code_repository/data/dual_cue_dyn_30.parquet"))
```
...
# Single cue 
```{r}
# classify top 2 performing single cues as "high performing"
top_2_single_cue <- si_opt.df %>% 
  filter(cue != "t") %>% 
  slice_max(n = 2, order_by = fitness_20) %>% 
  mutate(short_label = gsub("I\\+Ig", "Total I", short_label))
...
si_dc.high <- si_dc.df %>% filter(id %in% top_2_single_cue$id)
...
si_dc.high$short_label <- factor(si_dc.high$short_label, top_2_single_cue$short_label)
```

# Dual cue
```{r}
# get wide format dataframe. Here, we are interested in the RBC and total iRBC count
dual_dc.df <- dual_cue_dyn.df  %>% 
  mutate(
    short_label = paste0(gsub("_", " ", id), " & ", gsub("_", " ", id_b)),
...
# split into top and poor-performing cues
dual_dc.high <- dual_dc.df %>% filter(short_label %in% c("R log & I log", "R log & Total I log"))
...
# combine 
si_dc.high2 <- rbind(si_dc.high %>% select(time, short_label, R ,total), time_high.vir%>% select(time, short_label, R ,total))
```
...
```{r}
si_dc.plt <- ggplot() +
  geom_point(data = si_dc.high2 %>% filter(row_number() %% 1000 ==0), aes(x = total, y = R, color = short_label, shape = short_label), size = 3) +
   geom_path(data = si_dc.high2, aes(x= total, y = R, group = short_label, color = short_label), size = 1, arrow = arrow(type = "closed", angle = 10, length = unit(0.2, "inches"))) +
  scale_color_manual(values=c( "#fdcb44", "#fc8d59", "black"), name = "Cues")  +
...
dual_dc.plt <- ggplot() +
  ...
  geom_point(data = dual_dc.high %>% filter(row_number() %% 100 ==0), aes(x = total, y = R, color = short_label, shape = short_label), size = 2) +
  geom_path(data = time_high.vir, aes(x= total, y = R, group = short_label, color = short_label), size = 1, arrow = arrow(type = "closed", angle = 10, length = unit(0.2, "inches"))) +
   geom_path(data = dual_dc.high, aes(x= total, y = R, group = short_label, color = short_label), size = 1, arrow = arrow(type = "closed", angle = 10, length = unit(0.2, "inches"))) +
  scale_color_manual(values=c( "#4575b4",  "#91bfdb", "black"))  +
    labs(fill = "Cues", x = "Total iRBC", y = "RBC", color = "Cues") +
...
si_opt_area.df <- si_opt.df %>% 
  ...
  mutate(fitness_norm = fitness_20/9.883602,
        short_label = gsub("I\\+Ig", "Total I", short_label)) ## normalize fitness (use fitness obtained using time df=9 given that we are plotting dual cue and single on the same graph!)
...
```{r}
# split
dual_dc.ls <- split(dual_dc.df, dual_dc.df$short_label)

# get area
dual_dc.area <- cbind.data.frame(area = as.numeric(lapply(dual_dc.ls, get_dc_area)), short_label = names(lapply(dual_dc.ls, get_dc_area)))

# join with fitness
dual_cue_f_final_area.df <- dual_cue_f_final.df %>% 
  mutate(short_label = gsub("I\\+Ig", "Total I", paste(label, "&" , label_b))) %>% 
  left_join(dual_dc.area, by = c("short_label")) %>% 
...
# look at slope
summary(si_lm) ## -5.674e-13
summary(dual_lm) ## -3.185e-13
```
...
# single cue
## top 4 cues
si_opt_area.df_2 <- si_opt_area.df %>% filter(id %in% top_2_single_cue$id)

si_corr_area.plt1 <- ggplot() +
  geom_point(data = si_opt_area.df_2, aes(x = area, y = fitness_norm, color = short_label, shape = short_label), size = 3, alpha = 0.7) +
       scale_color_manual(values=c( "#fdcb44","#fc8d59"), name = "Cues") 
  
si_corr_area.plt2 <- si_corr_area.plt1 + ggnewscale::new_scale_color()

## the rest
si_corr_area.plt3 <- si_corr_area.plt2+
  geom_point(data = si_opt_area.df%>% filter(!id %in% top_2_single_cue$id), aes(x = area, y = fitness_norm), color = "grey", size = 3) +
  geom_point(aes(x = time_high.area, y = max(time_high.dyn %>% filter(variable == "tau_cum" & time == 20) %>% pull(value))/9.883602), shape = 15,size = 3, color = "black") +
   ggrepel::geom_label_repel(data = si_opt_area.df, aes(x = area, y = fitness_norm, label = short_label), box.padding
=0.1, size = 3, label.padding = 0.1, force_pull = 0.2, force = 30, show.legend = F) +
  #geom_line(data = si_lm.ci, aes(x = area, fit)) +
  #geom_ribbon(data = si_lm.ci, aes(x = area, ymin = lwr, ymax = upr), alpha = .15) +
  #annotate(geom = "text", label = "R^2=0.18; slope = -5.67e-13", x = 2*(10^11), y = 0.76) +
  annotate(geom = "text", label = "Time", x = 2.72*(10^11), y = 0.99) +
  labs(x = "Area", y = "Normalized fitness", shape = "Cue category", color = "Cue category") +
  #xlim(min(dual_cue_f_final_area.df$area), max(dual_cue_f_final_area.df$area)) +
  scale_x_continuous(trans = "log10") +
  theme_classic() +
  theme(legend.position = "none")
```
...
```{r}
# dual cue
dual_cue_f_final_area.df_4 <- dual_cue_f_final_area.df %>% filter(short_label %in% c("R log & I log", "R log & Total I log"))

dual_corr_area.plt1 <- ggplot() +
  geom_point(data = dual_cue_f_final_area.df_4, aes(x = area, y = fitness_norm, color = short_label, shape =short_label), size = 3, alpha = 0.7) +
       scale_color_manual(values=c("#4575b4",  "#91bfdb"), name = "Cues") 

dual_corr_area.plt2 <- dual_corr_area.plt1 + ggnewscale::new_scale_color()

dual_corr_area.plt3 <- dual_corr_area.plt2 +
  geom_point(data = dual_cue_f_final_area.df%>% filter(!short_label %in% c("R log & I log", "R log & Total I log")), aes(x = area, y = fitness_norm), size = 3, alpha = 0.7, color = "grey") +
  geom_point(aes(x = time_high.area, y = max(time_high.dyn %>% filter(variable == "tau_cum" & time == 20) %>% pull(value))/9.883602), shape = 15, size = 3, color = "black") +
  ggrepel::geom_label_repel(data = dual_cue_f_final_area.df %>% filter(short_label %in% c("R log & I log", "R log & Total I log")), aes(x = area, y = fitness_norm, label = short_label), box.padding
=0.1, size = 3, label.padding = 0.1, force_pull = 0.2, force = 50, show.legend = F, nudge_x = -0.01) +
  #geom_line(data = dual_lm.ci, aes(x = area, fit)) +
  #geom_ribbon(data = dual_lm.ci, aes(x = area, ymin = lwr, ymax = upr), alpha = .15) +
  #annotate(geom = "text", label = "R^2=0.11; slope = -3.19e-13", x = 2*(10^11), y = 0.76) +
  annotate(geom = "text", label = "Time", x = 2.72*(10^11), y = 0.9985) +
  labs(x = "Area", y = "Normalized fitness", shape = "Cue category", color = "Cue category") +
...
#---------assemble figure------------#
```{r}
si_dc_area.plt <- ggarrange(si_dc.plt, si_corr_area.plt3, ncol = 2, nrow = 1, align = "hv", common.legend = T, labels = c("A", "B"))
dual_dc_area.plt <- ggarrange(dual_dc.plt, dual_corr_area.plt3, ncol = 2, nrow = 1, align = "hv", common.legend = T, labels = c("C", "D"))
...
validation.df_p <- validation.df %>% 
  ...
  mutate(diff = fitness_20-V1,
         long_short_label = gsub("I\\+Ig", "Total I",paste0(long_label, " (", short_label, ")")))
