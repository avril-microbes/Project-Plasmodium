...
opt_local_beta <- function(df, default = F){
  ...
  if(default == F){write.csv(output, here(paste0("code_repository_updated/data/mc_burst_opt/", id, "_", beta_label, ".csv")))
  } else{
    write.csv(output, here(paste0("code_repository_updated/data/mc_burst_opt/", id, "_", beta_label, "_default.csv")))
    }
