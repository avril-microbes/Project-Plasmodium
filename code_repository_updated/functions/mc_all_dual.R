...
# posterior distribution). Note that this function is specfically written for dual cue MC simulations

mc_all_dual <- function(par, cue, cue_b, cue_range, cue_range_b, log, log_b, rand_df){
  
  # source single infection function
  source(here("code_repository_updated/functions/chabaudi_si_clean.R"))
  ...
  # write files
  write.csv(fitness.df, here(paste0("code_repository_updated/data/mc_all_fitness/", cue, "_", log, "-",cue_b, "_", log_b, "_", id, ".csv")))
  arrow::write_parquet(x = dyn_f, sink = here(paste0("code_repository_updated/data/mc_all_dyn/", cue, "_", log, "-",cue_b, "_", log_b, "_", id, ".parquet")))
}