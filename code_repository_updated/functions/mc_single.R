# This function executes single inefction simulation with only 1 parameter varying. 
mc_single <- function(par, cue, cue_range, log, rand_df){
  ...
  dyn_p.ls <- mapply(function(x, y){
    ...
    ## write df
    arrow::write_parquet(x = df_p, sink = here(paste0("code_repository_updated/data/mc_single_dyn/", id, "_", iter, "_", y, ".parquet")))
  },
  dyn.ls, c("rho", "beta", "psin", "psiw", "phin", "phiw"))
  
  # write fitness 
  write.csv(res, here(paste0("code_repository_updated/data/mc_single_fitness/", id, "_", iter, "_single.csv")))
}
