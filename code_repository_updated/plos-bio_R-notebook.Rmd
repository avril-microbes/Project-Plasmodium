...
```{r warning=FALSE}
# for single cues
source(here("code_repository_updated/functions/opt_local.R"))
source(here("code_repository_updated/functions/opt_global.R"))
source(here("code_repository_updated/functions/chabaudi_si_clean.R"))
source(here("code_repository_updated/functions/validate_si.R"))

# for optimization of static conversion rate
source(here("code_repository_updated/functions/chabaudi_si_static.R"))

# for dual cue
source(here("code_repository_updated/functions/dual_cue_opt.R"))
source(here("code_repository_updated/functions/dual_cue_opt_global.R"))

# for semi-stochastic single infection model
source(here("code_repository_updated/functions/mc_all.R"))
source(here("code_repository_updated/functions/mc_single.R"))
source(here("code_repository_updated/functions/mc_all_dual.R"))
source(here("code_repository_updated/functions/mc_single_dual.R"))
source(here("code_repository_updated/functions/opt_local_beta.R"))
```
...
si_opt.df <- read.csv(here("code_repository_updated/data/si_opt.csv"))
...
si_val.ls <- list.files(path = here("code_repository_updated/data/si_validation"), pattern = "*.csv", full.names = T)
...
# Dual cue optimization
#```{r}
# get rid of time from optimized datafrane
si_opt.df_no_t <- si_opt.df %>% filter(cue != "t")

#W get all unique combinations but eliminating all combinations that uses the same cue
dual_cue.df <- expand.grid(si_opt.df_no_t$id, si_opt.df_no_t$id) %>% 
  filter(Var1 != Var2) %>% 
  left_join(select(si_opt.df_no_t, cue1 = cue, id), by = c("Var1" = "id")) %>% 
  left_join(select(si_opt.df_no_t, cue2 = cue, id), by = c("Var2" = "id")) %>% 
  filter(cue1 != cue2) %>% 
...
cue_range_alt <- read.csv(here("code_repository_updated/data/cue_range_si_alt.csv")) 
...
posterior <- utils::read.csv(here("code_repository_updated/data/posterior.csv"))
...
source(here("code_repository_updated/functions/dual_cue_opt_beta.R"))

## import in dual cue fitness dataframe
dual_cue_f.df <- read.csv(here("code_repository_updated/data/dual_cue_fitness_final.csv"))
