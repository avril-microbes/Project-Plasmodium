...
## trying trapezoid estimation
much faster
```{r}
start.time <- Sys.time()
cl <- makeCluster(detectCores()); setDefaultCluster(cl = cl)
ch_si_i_i_20.opt_trap <- optimParallel(par = c(1,2,3,4), 
                      fn = chabaudi_si_opt_cpp, 
                      control = list(trace = 6),
                      immunity = "i",
                      parameters = parameters,
                      ...
                      cue = "I",
                      cue_range = cue_range)
ch_si_i_i_20.opt #-11.63003
stopCluster(cl)
# 1.768788 -362.923187  -37.759598    1.992568. Slightly different than original with precise integration
#1.769660 -363.080884  -37.980378    1.827227.
end.time <- Sys.time()
time.taken <- end.time - start.time 
time.taken #25.95028 mins. 67% of pure integration 
```

# trying vode method for dede
```{r}
start.time <- Sys.time()
cl <- makeCluster(detectCores()); setDefaultCluster(cl = cl)
ch_si_i_I_20.opt_vode_trap <- optimParallel(par = c(1,2,3,4), 
                      fn = chabaudi_si_opt_cpp, 
                      control = list(trace = 6),
                      immunity = "i",
                      parameters = parameters,
                      ...
                      cue = "I",
                      cue_range = cue_range)
ch_si_i_I_20.opt_vode_trap$par #1.766034 -360.630602  -36.766177 1.477862, with #-11.63002. Very small difference between lsoda 
stopCluster(cl)
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken # 8.007386 minute much faster

# plot. Not huge difference
ch_si_i_I_20_vode_trap.df <- par_to_df(ch_si_i_I_20.opt_vode_trap, cue_range)

    ggplot() +
    geom_line(data = ch_si_i_I_20_cr.df, aes(x = cue_range, y = model), alpha = 0.7, color = "blue") +
    geom_line(data = ch_si_i_i_20_cr.df, aes(x = cue_range, y = model), color = "red", alpha = 0.7) +
    xlab("Infected RBC density") +
    ylab("Transmission investment") +
    labs(color = "Optimization methods") +
    theme_bw()
...
# trying with vode and original integration
```{r}
# Line search cannot locate an adequate point after 20 function and gradient evaluations. Bad direction in the line search
start.time <- Sys.time()
cl <- makeCluster(detectCores()); setDefaultCluster(cl = cl)
ch_si_i_I_20.opt_vode_int <- optimParallel(par = c(1,2,3,4), 
                      fn = chabaudi_si_opt_cpp, 
                      control = list(trace = 6),
                      immunity = "i",
                      parameters = parameters,
                      ...
                      cue = "I",
                      cue_range = cue_range)
ch_si_i_I_20.opt_vode_int$par
stopCluster(cl) # 1.768348 -363.405382  -34.474332   -4.695846. Very different. due to bad solve
end.time <- Sys.time()
time.taken <- end.time - start.time #12.76853 mins
time.taken
```
