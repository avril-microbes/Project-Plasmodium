...
get_dual_rn <- function(df){
  ...
  if(isTRUE(str_detect(cue, "\\+", negate = T)) & isTRUE(str_detect(cue_b, "\\+", negate = T))){
    df_f <- df %>% 
      filter(variable %in% c(cue, cue_b)) %>% 
    ...
    df_f1 <- df %>% 
      filter(variable == non_combined_cue)
    ...
    df_f2 <- df %>% 
      ...
      group_by(variable, time) %>% ## for each time point, group the variables
