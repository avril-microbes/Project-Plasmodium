...
## save data
si_opt.df %>% 
  select(long_label, fitness = fitness_20, fitness_normalized = fitness_norm) %>% 
  write.csv(here("code_repository/supplementary_data/fig2A_1.csv"), row.names = F)

si_dyn_cr_fitness.df %>% 
  select(long_label, time, transmission_investment = value) %>% 
  write.csv(here("code_repository/supplementary_data/fig2A_2.csv"), row.names = F)
```
...
get_si_rn <- function(df){
  ...
  rn <- par_to_df(par = par, cue_range = cue_range, max = df$high)
...
## write data
si_rn.df_p %>% 
  mutate(long_label = paste(cue_label, log)) %>% 
  select(long_label, cue_value = cue_range, transmission_investment = cr) %>% 
  write.csv(here("code_repository/supplementary_data/fig2B.csv"))
```
...
fitness_rn.plt <- ggplot() +
  ...
  mutate(rel_cue = round(cue_range/(max-min)*100)) %>% 
   ...
   geom_rug(data = si_rug_cue.df_none, aes(x = value), color = "black", sides = "t", length = unit(0.1, "npc")) +
  geom_rug(data = si_rug_cue.df_log, aes(x = value), color = "#D41159", sides = "b", length = unit(0.1, "npc")) +
  facet_wrap(~cue_label, scales = "free_x", ncol = 1) +
  labs(x = "Cue range", y = "Transmission investment", color = "Cue", shape = "Cue") +
  ylim(0, 1) +
...
```{r}
ggarrange(fitness_rank_cr.plt, fitness_rn.plt, widths = c(2.3, 1), align = "h", ncol = 2, nrow = 1, labels = c("A", "B"))
ggsave(units = "px", dpi = 300, width = 2250/1.25, height = 1700/1.25, filename = here("code_repository/figures/fitness_rn.tiff"), bg = "white", scale = 1.25, compression = "lzw")
```
...
## write data
dual_si_fitness.df %>% 
  select(label = label_comb, dual_cue_fitness_normalized = fitness_dual_norm, best_single_cue_fitness_normalized = fitness_si_final) %>% 
  write.csv(here("code_repository/supplementary_data/fig3B.csv"))
```
...
## write data
dual_selected_cr.df %>% 
  select(label = label_new, time, transmission_investment = value) %>% 
  write.csv(here("code_repository/supplementary_data/fig3C.csv"))
```
...
## write data
Rlog_Ilog.hm %>% 
  select(asexual_iRBC_log = cue_range_b, RBC_log = cue_range, transmssion_investment = cr) %>%
  write.csv(here("code_repository/supplementary_data/fig3D.csv"))
...
## write csv
dual_cue_cr.df %>% 
  select(label = label_comb, time, transmission_investment = value) %>% 
  write.csv(here("code_repository/supplementary_data/S7_Data.csv"), row.names = F)
```
...
ggsave(here("code_repository/figures/dual_cue_cr.tiff"), units = "px", width = 2000, height = 1500, dpi=300,  bg = "white", compression = "lzw")
```
...
# write data
dual_selected_taucum.df_p %>% select(label = label_new, time, cumulative_fitness_normalized = fitness_norm) %>% write.csv(here("code_repository/supplementary_data/S8_Data.csv"), row.names = F)
...
ggsave(here("code_repository/figures/dual_taucum.tiff"), units = "px", width = 1600, height = 1000, dpi=300,  bg = "white", scale = 1.2, compression = "lzw")
...
# write data
exp_ss.df %>% 
  select(Study, Strain, Dose, Mouse, Gametocyte, iRBC, RBC) %>% 
  write.csv(here("code_repository/supplementary_data/fig4_exp.csv"))
``` 
...
ggsave(here("code_repository/figures/exp_disease-curve.tiff"), units = "px", width = 2250, height = 1500, scale = 1.4, dpi=300,  bg = "white", compress = "lzw")
```
...
## write csv
dual_cue_dyn.df %>% 
  select(cue_1 = label, cue_2 = label_b, time, variable, density = value) %>% 
  write.csv(here("code_repository/supplementary_data/fig4_sim.csv"))
...
ggsave(here("code_repository/figures/sim_disease-curve.tiff"), units = "px", width = 2250, height = 3000, scale = 1.5, dpi=300,  bg = "white", compression = "lzw")
```
...
mc_all_fitness.df <- read_parquet(here("code_repository/data/mc_all_fitness.parquet"))
```
...
mc_all_fitness.df_p %>% 
  filter(iter != 1) %>% 
  left_join(select(mc_all_fitness_ref.df, short_label, fitness_deterministic = fitness, cue_condition), by = "short_label") %>% 
  select(label = short_label, iter, fitness_semi_stochastic = fitness, fitness_deterministic) %>% 
  write.csv(here("code_repository/supplementary_data/fig5A.csv"))
```

# plot
```{r}
mc_all_fitness.pl <- ggplot() +
  ggrepel::geom_label_repel(data = mc_all_fitness_rand.df, aes(x = fitness/9.883602, y = geom_mean, label = short_label, colour = factor(cue_condition)),box.padding
=0.1, size = 3, label.padding = 0.1, force_pull = 0.2, force = 30) +
  geom_point(data = mc_all_fitness_rand.df, aes(x = fitness/9.883602, y = geom_mean, shape = cue_condition, color = cue_condition), size = 2) +
...
# write data
mc_ilog_dyn.df_cue %>% 
  select(time, iter, RBC_log = R, iRBC_log = I, transmission_investment = cr) %>% 
  write.csv(here("code_repository/supplementary_data/fig5B_1.csv"))

# write data
mc_rlog_ilog_dyn.df_cue %>% 
  select(time, iter, RBC_log = R, iRBC_log = I, transmission_investment = cr) %>% 
  write.csv(here("code_repository/supplementary_data/fig5B_2.csv"))
```
...
## write csv
posterior.long %>% 
  select(label, iter, parameter, value) %>% 
  write.csv(here("code_repository/supplementary_data/S9_Data.csv"), row.names = F)
```
...
## save
ggsave(units = "px", dpi = 300, width = 2000, height = 1300, filename = here("code_repository/figures/posterior.tiff"), bg = "white", scale = 1, compression = "lzw")
```
...
# write data
mc_single_fitness.long2 %>% 
  select(cue = cue_label, paramater_varied = parameter_label, iter, geometric_mean_fitness = geo_mean, fitness_semi_stochastic_normalized = fitness_norm, fitness_deterministic_normalized = fitness_det_norm) %>% 
  write.csv(here("code_repository/supplementary_data/S10_Data.csv"), row.names = F)
```
...
ggsave(units = "px", dpi = 300, width = 2500, height = 2000, filename = here("code_repository/figures/mc_single_fitness.tiff"), bg = "white", scale = 1,compression = "lzw")
```
...
# write data
si_dc.high2 %>% 
  select(label = short_label, time, RBC = R, total_iRBC = total) %>% 
  write.csv(here("code_repository/supplementary_data/fig6A.csv"))

dual_dc.high %>% 
  select(label = short_label, time, RBC = R, total_iRBC = total)%>% 
  write.csv(here("code_repository/supplementary_data/fig6C.csv"))
```
...
# save data
si_opt_area.df %>% 
  select(label = short_label, fitness_normalized = fitness_norm, area) %>% 
  write.csv(here("code_repository/supplementary_data/fig6B.csv"))

dual_cue_f_final_area.df%>% 
  select(label = short_label, fitness_normalized = fitness_norm, area) %>% 
  write.csv(here("code_repository/supplementary_data/fig6D.csv"))
```
...
# write data
validation.df_p %>% 
  select(label = long_short_label, fitness_optimized = fitness_20, fitness_randomized = V1) %>% 
  write.csv(here("code_repository/supplementary_data/S6_Data.csv"), row.names = F)
```
...
ggsave(units = "px", dpi = 300, width = 2550, height = 1500, filename = here("code_repository/figures/si_validation.tiff"), bg = "white", scale = 1,compression = "lzw")
```

#----------------------------------------#
# Further plots for revision
#----------------------------------------#
# Differences in dynamics between different transformations
```{r}
# keep only relevant variables
time_high.cr_f <- time_high.cr %>% 
  filter(variable %in% c("I") &
           time<8 & time>0)

# plot different transformations
## untransformed
time_dyn_pl.1 <- ggplot(time_high.cr_f, aes(x = time, y = value)) +
  geom_line() +
...
## log transformed
time_dyn_pl.2 <- ggplot(time_high.cr_f %>% mutate(log_value = log10(value),
                                 log_value_2 = ifelse(log_value<0, 0, log_value)), aes(x = time, y = log_value_2)) +
  geom_line() +
 # facet_wrap(~variable, scales = "free_y") +
  theme_bw() +
  theme(legend.position = "bottom",
        ...
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +
  labs(x = "Time (days)", y = "log10 Density\n(cells/µL)")

## sqrt transformed
time_dyn_pl.3 <- ggplot(time_high.cr_f %>% mutate(sqrt_value = sqrt(value)), aes(x = time, y = sqrt_value)) +
  geom_line() +
 # facet_wrap(~variable, scales = "free_y") +
  theme_bw() +
  theme(legend.position = "bottom",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  labs(x = "Time (days)", y = "sqrt Density\n(cells/µL)")

## arcsine
time_dyn_pl.4 <- ggplot(time_high.cr_f %>% 
         group_by(variable) %>% 
         mutate(max= max(value, na.rm = T)) %>% 
         ungroup() %>% 
         mutate(arcsine_value = asin(sqrt(value/max))), 
       aes(x = time, y = arcsine_value)) +
  geom_line() +
  #facet_wrap(~variable, scales = "free_y") +
  theme_bw() +
  theme(legend.position = "bottom",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) +
  labs(x = "Time (days)", y = "Arcsine Density\n(cells/µL)")

# plot together
ggarrange(time_dyn_pl.1, time_dyn_pl.2, time_dyn_pl.3, ncol = 1, nrow = 3, heights = c(1,1,1.3), align = "v",
           labels = c("Raw", "Log10", "Sqrt", "Arcsine"), hjust = 0, vjust = 1)

ggsave(here("code_repository/figures/time_dynamics_transformation.png"), height = 5, width = 3)
```
