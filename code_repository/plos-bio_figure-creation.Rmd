...
ggplot() +
  geom_line(data = dual_selected_taucum.df, aes(color = label_new, x = time, y = value), size = 1) +
  geom_point(data = dual_selected_taucum.df %>% filter(time%%1 == 0), aes(color = label_new, x = time, y = value, shape = label_new), size = 3) +
  labs(x = "Time (days)", y = "Cumulative transmission potential", color = "Cue(s)", shape = "Cue(s)") +
  xlim(0, 20) +
  scale_color_manual(values = c(orange,"#E1BE6A",blue ,"black")) +
  theme_classic() +
  theme(legend.position="right") +
