...
mc_all_fitness.df_p <- mc_all_fitness.df %>% 
  ...
  mutate(long_label = ifelse(cue == "R log & I log", "RBC log &\nasexual iRBC log", long_label)) ## assign label manually to dual cue data
...
par_det.df <- data.frame(
  ...
  deterministic = c(2.627156e-01, log10(3.520591e-02), log10(5.508420e+02), log10(1.669234e+01), 8.431785e-01, 5.721000e+00 )
)

## make into long format
posterior.long <- posterior.df %>% 
  mutate(iota_N1 = log10(iota_N1),
         phi_N1 = log10(phi_N1),
         phi_N2 = log10(phi_N2)) %>% 
  ...
  mutate(label = case_when(
    parameter == "rho" ~ "RBC replenishment (ρ)",
    parameter == "phi_N1" ~ "log10 Half-life indis (ϕn)",
    parameter == "phi_N2" ~ "log10 Half-life targeted (ϕw)",
    parameter == "iota_N1" ~ "log10 Activation indis (ψn)",
