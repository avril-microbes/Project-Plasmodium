...
get_mc_fitness_diff <- function(df, dual = F){
  ...
  if(isFALSE(dual)){
    fitness_ls <- lapply(parameter_tsukushi_ls, function(x){
      fitness <- chabaudi_si_clean(parameters_cr = par, 
      parameters = x, 
      immunity = "tsukushi",
      time_range = seq(0, 20, 0.01), 
      cue = cue, 
...
mc_burst_diff.pl <- ggplot() +
  geom_violin(data = mc_burst_final_diff.df %>% filter(beta_optimized != beta_simulated), 
             aes(x = short_label, y = fitness), color = "transparent", fill = "light grey") +
  geom_jitter(data = mc_burst_final_diff.df %>% filter(beta_optimized != beta_simulated), 
...
mc_burst_rn_RI.df_p <- mc_burst_rn_RI.df %>% 
  ...
  left_join(mc_burst_bound_RI %>% filter(variable == "I") %>% 
              select(min_b = min, max_b = max, id, beta), by = c("id", "beta")) %>% 
  filter(cue_range >= min & cue_range <= max &
           cue_range_b >= min_b & cue_range_b <= max_b)
...
```{r}
## add short form of label for consistency purposes
mc_burst_rn.df_lab <- mc_burst_rn.df %>% 
  left_join(select(ez_label, id, short_label), by = "id") %>% 
  mutate(long_short_label = paste0(long_label, " (", short_label, ")"))

mc_burst_rn_RI.df_lab <- mc_burst_rn_RI.df %>% 
  mutate(long_short_label = "RBC log & asexual iRBC log\n(R log & I log)")

## for dual cue models, it is worth calculating the amount of variance (standard variation at each cue)
mc_burst_rn_RI.sd <- mc_burst_rn_RI.df_lab %>% 
  group_by(cue_range, cue_range_b) %>% 
  summarise(sd = sd(cr, na.rm = T),
            n = n())
```
...
## Plot
```{r}
### for the main figure, we will only plot the top 2 and bottom 2 cues ranked by geometric means across all optimization (so section B ranking)
mc_burst_rn.pl1 <- ggplot(mc_burst_rn.df_lab %>% filter(id == "I+Ig_log")) +
  geom_line(aes(x = cue_range, y = cr, color = beta, group = beta)) +
  scale_color_viridis_c() +
  labs(x = "Cue range", y = "Conversion rate", color = "Burst size", subtitle = "Total iRBC log (I+Ig log)") +
  theme_bw() +
  theme(legend.position = "none",
    panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
mc_burst_rn.pl2 <- ggplot(mc_burst_rn.df_lab %>% filter(id == "I_log")) +
  geom_line(aes(x = cue_range, y = cr, color = beta, group = beta)) +
  scale_color_viridis_c() +
  labs(x = "Cue range", y = "Conversion rate", color = "Burst size", subtitle = "Asexual iRBC log (I log)") +
  theme_bw() +
  theme(legend.position = "none",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
mc_burst_rn.pl3 <- ggplot(mc_burst_rn.df_lab %>% filter(id == "I+Ig_none")) +
  geom_line(aes(x = cue_range, y = cr, color = beta, group = beta)) +
  scale_color_viridis_c() +
  labs(x = "Cue range", y = "Conversion rate", color = "Burst size", subtitle = "Total iRBC (I+Ig)") +
  theme_bw() +
...
mc_burst_rn.pl4 <-ggplot(mc_burst_rn_RI.sd) +
  geom_raster(aes(y = cue_range, x = cue_range_b, fill = sd)) +
  scale_fill_viridis_c(option = "B") +
  labs(x = "Asexual iRBC log10", y = "RBC log10", subtitle = "RBC log & asexual iRBC log (R log & I log)") +
  theme_bw() +
  theme(legend.position = "bottom",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
## arrange the reaction norms
mc_burst_rn.pl <- ggarrange(mc_burst_rn.pl1, mc_burst_rn.pl2, mc_burst_rn.pl3, mc_burst_rn.pl4, ncol = 2, nrow = 2, align = "hv", heights = c(0.45,0.6))

## put them together
ggarrange(mc_burst_fitness_com.pl, mc_burst_rn.pl, ncol = 2, align = "hv", labels = c("", "C"), widths = c(0.5,0.55))
ggsave(units = "px", dpi = 300, width = 2600, height = 1600, filename = here("code_repository/figures/mc_burst_main.tiff"), bg = "white", scale = 1.25)
```
