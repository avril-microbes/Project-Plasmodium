...
mc_all_fitness.df <- mc_all_fitness_dc.df %>% 
  mutate(iter = id,
         id = paste0(cue, "_", log, "-", cue_b,"_", log_b)) %>%
...
mc_single_sc.ls <- mc_single.ls[!grepl("-", basename(mc_single.ls))]
mc_single_dc.ls <- mc_single.ls[grepl("-", basename(mc_single.ls))]

## check number of files
length(mc_single_sc.ls)
length(mc_single_dc.ls)

## read and combine
mc_single_sc_fitness.df <- do.call(rbind, mclapply(mc_single_sc.ls, function(x) read.csv(x), mc.cores = 6))
mc_single_dc_fitness.df <- do.call(rbind, mclapply(mc_single_dc.ls, function(x) read.csv(x), mc.cores = 6))

# look at # of reps
mc_single_sc_fitness.df %>% 
  group_by(cue, log) %>% 
  tally()

mc_single_dc_fitness.df %>% 
  group_by(cue, log, cue_b, log_b) %>% 
  tally()

## combine
mc_single_fitness.df <- mc_single_dc_fitness.df %>% 
  mutate(iter = id,
         id = paste0(cue, "_", log, "-", cue_b,"_", log_b)) %>%
  select(-c(cue, cue_b, log, log_b)) %>% 
  rbind(
    mc_single_sc_fitness.df %>% 
      mutate(iter = id,
             id = ifelse(cue != "t", paste0(cue, "_", gsub("log10", "log", log)), "time")
...
mc_single_fitness.long <- mc_single_all_fitness.df %>% 
  ...
  select(id, fitness_rho, fitness_beta, fitness_psin, fitness_psiw, fitness_phin, fitness_phiw, iter, fitness_all, fitness_ref) %>% # keep only fitness and associated labels
...
mc_single_fitness.sum <- mc_single_fitness.long_p %>% 
  group_by(short_label, parameter) %>% 
...
mc_single_fitness.long_p_f$short_label <- factor(mc_single_fitness.long_p_f$short_label, mc_all_fitness_sum.df %>% arrange(desc(geom_mean)) %>% pull(short_label))
...
posterior.ls[1:5]
mclapply(posterior.ls[1:5], function(x){
...
test <- arrow::read_parquet(here("code_repository/data/mc_single_dyn/I_none_2_phin.parquet"))
test
```
