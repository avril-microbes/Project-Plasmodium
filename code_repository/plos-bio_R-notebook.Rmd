...
```{r}
# time
ci_opt_t <- co_infection_opt_alt(parameters_cr = rep(0.5,4),
                             limit = limit,
...
Rlog_Ilog_burst.df <- dual_cue_f.df %>% 
  filter(id == "R_log" & id_b == "I_log") %>% 
  mutate(log = "log", log_b = "log", cue = "R", cue_b = "I",
         low = 6, high = 7, low_b = 0, high_b = 6.77815125) %>% 
  cbind(beta = beta_ls)

## split
Rlog_Ilog_burst.ls <- split(Rlog_Ilog_burst.df, seq(nrow(Rlog_Ilog_burst.df)))

## run function
lapply(Rlog_Ilog_burst.ls, function(x){dual_cue_opt_beta(x)})
lapply(Rlog_Ilog_burst.ls, function(x){dual_cue_opt_beta(x, default = T)})
```
