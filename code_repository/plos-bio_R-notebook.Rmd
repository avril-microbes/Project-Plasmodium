...
dual_cue_init_glb <- lapply(dual_cue.ls3, dual_to_si_glb)
...
This set gives us a lower fitness when compared to Ig log alone, indicating issues with optimization. Here, I will use global optimization, where one of the starting population has the initial parameter optimized to resemble Ig log (see round 3 for more details)

```{r}
dual_cue.ls3[[6]]
dual_to_si(dual_cue.ls3[[6]] %>% mutate(fitness_si_b = 0))
```
...
# creating starting population matrix. Here,  we are sampling 89 (total population -1) random strategies from our defined parameter bounds in the dual cue optimization model!
set.seed(137)
rlog_iglog_init_pop <- rbind(cbind(
  sample(-10:10,89, replace=T),
  sample(-500:500,89, replace=T),
  sample(-1000:1000,89, replace=T),
  sample(-1000:1000,89, replace=T),
  sample(-250:250,89, replace=T),
  sample(-500:500,89, replace=T),
  sample(-1000:1000,89, replace=T),
  sample(-500:500,89, replace=T),
  sample(-250:250,89, replace=T)
  ),
  c(0.001375478,	-1.33821661,	1.642795646	,1.885708906,	-2.724031203,	1.315704257	,1.876724443,	-1.132578883,	1.147012481 ## adding best parameter set
))

rlog_iglog_init_pop <- matrix(rep(t(c(0.001375478,	-1.33821661,	1.642795646	,1.885708906,	-2.724031203,	1.315704257	,1.876724443,	-1.132578883,	1.147012481)),90),ncol=9,byrow=TRUE)

rlog_iglog_init_pop
# run dual cue optimization
dual_cue_opt_global(dual_cue.ls[[29]])
