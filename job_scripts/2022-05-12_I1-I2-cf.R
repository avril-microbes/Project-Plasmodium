...
res <- foreach(i= 1:1000, .verbose = T, .packages = c("doParallel", "doRNG", "deSolve", "splines2", "stringr", "dplyr", "tidyr", "crone", "here")) %dorng% {
  ...
  fitness <- chabaudi_ci_clean(parameters_cr_1 = c(0.9999999,   -9.1650326,  -71.7214007, 4998.7943898 ),
...
## rbind
fitness.df <- as.data.frame(cbind(fitness.df, label = rep("Total asexual iRBC", nrow(fitness.df))))
write.csv(fitness.df, here("validation/I1-I2-cf.csv"))
