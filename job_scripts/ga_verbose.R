# custom function based upon the GA function from the GA package
# following modifications are made:
  # writes log file based on specified "id". This is useful for submitting to large clusters
# for reference, population = 100 ~ 30 seconds per iteration on a full node with 48 cores
# population = 500 ~ 1.5 minutes per iteration (scales linearly)
##############################################################################
#                                                                            #
...
ga_verbose <- function(type = c("binary", "real-valued", "permutation"), 
               fitness, ...,
               lower, upper, nBits,
               ...
               id = NULL) 
{
  ...
  # start log
  log_path = file.path(here::here(paste0(id, "_ga.log")))
  log <- logr::log_open(log_path)
  ...
  for(iter in seq_len(maxiter))
  {
    ...
    { 
      ...
      # this prints log in consol
      cat(object@bestSol[[iter]])
      # write log file
      logr::log_print(list(object@bestSol[[iter]], max(object@fitness)))
    }
