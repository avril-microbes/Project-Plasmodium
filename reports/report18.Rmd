...
ci_comp_static.mean <- ggplot() +
  geom_bar(data =  ci_comp_static.df3, aes(y = id_1_alt, x = mean), stat = "identity") +
...
ci_static.pl <- ggarrange(ci_comp_static.map, ci_comp_static.mean, align = "h", widths = c(1, 0.2))
ggsave(here("figures/report18/ci_comp_static.png"), width = 10)
```
...
# get fitness (mean)
invasion.df3 <- invasion.df2 %>% 
  group_by(mut_alt) %>% 
  summarize(mean = mean(fitness))

ci_invade.pl1 <- ggplot(data = invasion.df2, aes(x = res_alt, y = mut_alt, fill = fitness))+
  geom_tile(color = "black") +
  scale_fill_gradient2(low = "red", high = "blue", mid = "white", 
   midpoint = 0, space = "Lab", lim = c(-1.8, 1.5), name="Fitness\ndifference") +
  theme_minimal() +  
...
ci_invade.pl2 <- ggplot() +
  geom_bar(data =  invasion.df3, aes(y = mut_alt, x = mean), stat = "identity") +
  labs(y = "", x = "Mean fitness\ndifference") +
  theme_bw() +
  theme(plot.margin = margin(l = 0),
       panel.grid.major = element_blank(),
...
ggsave(here("figures/report18/ci_invade.png"), width = 10)

```

```{r}
test <- left_join(ci_comp_static.df, invasion.df2, by = c("id_1_alt" = "mut_alt",
                                                          "id_2_alt"= "res_alt"))
test %>% 
...
ggplot() +
  geom_point(data = test, aes(x = fitness_diff, y = fitness, color = 
id_1_alt)) +
