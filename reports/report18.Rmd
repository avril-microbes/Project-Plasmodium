...
ci_opt_f.df <- ci_opt.df %>% filter(id == "Ig-i_log" | id == "G1+G2_none")

# split for confirmatory checks
ci_opt_f.ls <- split(ci_opt_f.df, seq(nrow(ci_opt_f.df)))

# run confirmatory checks
ci_opt_f.cf <- lapply(ci_opt_f.ls, check_ci_opt)

ci_opt_f.res <- do.call(rbind, ci_opt_f.cf)
write.csv(ci_opt_f.res, here("data/ci_opt_cf/ci_opt_res_f.csv"), row.names = F)
ci_opt_f.res <- read.csv(here("data/ci_opt_cf/ci_opt_res_f.csv"))

# total gametocyte is still bad. Sexual iRBC log10 is ok 
ci_opt_f.res %>% group_by(label) %>% summarise(min = min(V1))
```
