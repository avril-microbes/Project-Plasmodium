...
### Sanity checks to see if we have successfully optimized time-based co-infection strategy. it seems that for no immunity model, starting from single infection helps. However, for our models that include immunity, the different starting points make little differences
```{r}
# sanity checks. Should recaptitulate Greischar et al's co-infection model without immunity
## Greischar et al 2016 (ni, single infection as starting value): 11.80 -23.58 -3.97 -16.15
## ni, tskushi parameter value, starting parameter 0.5x4: 11.95826 -28.01326  12.97268 -49.61479 
## ni, greischar parameter value, starting parameter 0.5x4: 9.404444 -25.422225  22.104180 -70.732451
## ni, greischar parameter value, starting parameter value 4.55386, -13.0056, 4.15466, -11.9424 (best single infection):8.226041 -18.188289   3.769436 -17.745308
## tsukushi, tsukushi parameter value, starting parameter 4.55386, -13.0056, 4.15466, -11.9424 (best single infection):31.35883  -86.98317   70.37316 -217.06454
## tsukushi immunity, tsukushi parameter value, startint parameter: 8.226041, -18.188289,   3.769436 ,-17.745308 (greischar et al with lag): 30.02169  -83.75933   68.36241 -208.66380 (no difference!)

parameters_ni <- c(R1 = 8.5*10^6, # down from 8.89*10^6
                lambda = 3.7*10^5,
                mu = 0.025, 
                p = 4*10^-6, # down from 8*10^-6
                alpha = 1, 
                alphag = 2, 
                beta = 10, # up from 5.721
                mum = 48, 
                mug = 4, 
                I0 = 43.85965, 
                Ig0 = 0, 
...
ci_opt_t.sc <- co_infection_opt_alt(parameters_cr = c(4.55386, -13.0056, 4.15466, -11.9424),
                             limit = limit,
...
## simulate and get conversion rate dynamics
### default time as parameter (tsukushi immunity after co-infection opt)
...
ci_t.df2 <- chabaudi_ci_clean(parameters_cr_1 = c(31.35883,  -86.98317 ,  70.37316, -217.06454),
                            parameters_cr_2 = c(31.35883,  -86.98317 ,  70.37316, -217.06454),
                            immunity = "tsukushi",
                            parameters = parameters_tsukushi,
                            time_range = time_range, 
                            cue_1 =  "t",
...
ci_t_sc.df <- chabaudi_ci_clean(parameters_cr_1 = c(11.95826, -28.01326 , 12.97268, -49.61479),
                            parameters_cr_2 = c(11.95826, -28.01326 , 12.97268, -49.61479),
                            immunity = "ni",
                            parameters = parameters_tsukushi,
                            time_range = time_range, 
                            cue_1 =  "t",
...
ci_t_sc.df2 <- chabaudi_ci_clean(parameters_cr_1 = c(9.404444, -25.422225 , 22.104180 ,-70.732451),
                            parameters_cr_2 = c(9.404444, -25.422225 , 22.104180 ,-70.732451),
                            immunity = "ni",
                            parameters = parameters_ni,
                            time_range = time_range, 
                            cue_1 =  "t",
                            cue_2= "t",
                            cue_range_1 = time_range, # cue range of strain 1
...
ci_t_sc.df3 <- chabaudi_ci_clean(parameters_cr_1 = c(8.226041, -18.188289,   3.769436, -17.745308),
                            parameters_cr_2 = c(8.226041, -18.188289,   3.769436, -17.745308),
                            immunity = "ni",
                            parameters = parameters_ni,
                            time_range = time_range, 
                            cue_1 =  "t",
...
## best parameter values differ based on initial starting values. Here, we are going to compare the performance of the values derived from 0.5x4 vs single infection best paramters. -0.07071394. Here, it seems that using the initial si strategy as starting value is better
chabaudi_ci_clean(
  parameters_cr_1 = c(9.404444, -25.422225 , 22.104180 ,-70.732451),
  parameters_cr_2 = c(8.226041, -18.188289,   3.769436, -17.745308),
  immunity = "ni",
  parameters = parameters_ni,
  time_range = time_range, 
...
### for tsukushi immunity model, compare fitness of optimized from si vs optimized from 0.5x4. 0.0074109. Not huge difference
chabaudi_ci_clean(
  parameters_cr_1 = c(31.35883,  -86.98317,   70.37316, -217.06454),
  parameters_cr_2 = c(26.16425,	-71.07799,	53.34121,	-166.25693),
  immunity = "tsukushi",
  parameters = parameters_tsukushi,
  time_range = time_range, 
...
## plot
ggplot() +
  geom_line(data = ci_t_sc.df %>% filter(variable == "cr_1"),
            aes(x = time, y = value, color = "no immunity, tsukushi, 0.5x4")) +
  geom_line(data = ci_t_sc.df2 %>% filter(variable == "cr_1"),
            aes(x = time, y = value, color = "no immunity, greischar, 0.5x4")) +
  geom_line(data = ci_t_sc.df3 %>% filter(variable == "cr_1"),
            aes(x = time, y = value, color = "no immunity, greischar, si")) +
  geom_line(data = ci_t.df %>% filter(variable == "cr_1"),
            aes(x = time, y = value, color = "immunity, tsukushi, 0.5x4")) +
  geom_line(data = ci_t.df2 %>% filter(variable == "cr_1"),
            aes(x = time, y = value, color = "immunity, tsukushi, si")) +
  labs(y = "Conversion rate") +
  theme_classic()

ggsave(here("figures/plos-bio/ci_sc.png"), height = 4)
```

# get time-based optimization for both co-infection and single-infection.
```{r}
...
#===========================================================#
# additional infection dynamics figure for Nicole
#===========================================================#
Here, we need to plot, for the top 3 performing single and co-infection cues, the dynamics of RBC, log10 total iRBC, log10 gametocytes, N, and W

# process dataset
filter out dynamics of top performing cues
```{r}
# get the ids of the top 3 performing cues (plus time). we are going to append time within them
si_top4 <- si_fitness.df %>% 
  select(id, value) %>% 
  rbind(data.frame(id = "time", value = 9.787899)) %>% 
  arrange(desc(value)) %>% 
  slice_head(n=4) %>% select("id") %>% unlist()

ci_top4 <- ci_fitness.df %>% 
  select(label, value) %>% 
  rbind(data.frame(label = "time", value = 2.311841)) %>% 
  arrange(desc(value)) %>% 
  slice_head(n=4) %>% select("label") %>% unlist()


# filter dynamics data to only include those within the top list
si_dyn.top <- si_dyn.df %>% rbind(cbind(si_t.df, id = "time")) %>% 
  filter(id %in% si_top4 & variable %in% c("R", "I", "Ig", "G", "N", "W", "tau") & time < 20)

ci_dyn.top <- ci_dyn.df %>% 
  rbind(cbind(ci_t.df, label = "time")) %>% 
  filter(label %in% ci_top4 & variable %in% c("R", "I1", "Ig1", "G1", "N", "W", "tau1") & time < 20)


# calculate total IRBC (I+Ig), log transform the iRBC variables
si_dyn.top_p <- si_dyn.top %>%
  tidyr::pivot_wider(names_from = variable, values_from = value, id_cols = c(time, id)) %>% 
  mutate(I_Ig = log10(I+Ig),
         G = log10(G+1)) %>% 
  select(-c(I, Ig)) %>% 
  tidyr::pivot_longer(-c(time, id), names_to = "variable") %>% 
  mutate(variable2 = case_when(
    variable == "R" ~ "RBC",
    variable == "I_Ig" ~ "Total iRBC",
    variable == "N" ~ "Nondis immunity",
    variable == "W" ~ "Targeted immunity",
    variable == "G" ~ "Gametocyte",
    variable == "tau" ~ "Fitness"
  ))

ci_dyn.top_p <-  ci_dyn.top %>%
  select(time, variable, value, label) %>% 
  tidyr::pivot_wider(names_from = variable, values_from = value, id_cols = c(time, label)) %>% 
  mutate(I_Ig = log10(I1+Ig1+1),
         G1 = log10(G1+1)) %>% 
  select(-c(I1, Ig1)) %>% 
  tidyr::pivot_longer(-c(time, label), names_to = "variable") %>% 
  mutate(variable2 = case_when(
    variable == "R" ~ "RBC",
    variable == "I_Ig" ~ "Total iRBC",
    variable == "N" ~ "Nondis immunity",
    variable == "W" ~ "Targeted immunity",
    variable == "G1" ~ "Gametocyte",
    variable == "tau1" ~ "Fitness"
  ))
```

# plot dynamics
```{r}
si_dyn_top.pl <- ggplot(data = si_dyn.top_p) +
  geom_line(aes(x = time, y = value, color = id)) +
  facet_wrap(~variable2, scales = "free", ncol = 3) +
  labs(x = "Time", y  = "Value", color = "Cue") +
  theme_classic2()

ci_dyn_top.pl <- ggplot(data = ci_dyn.top_p) +
  geom_line(aes(x = time, y = value, color = label)) +
  facet_wrap(~variable2, scales = "free", ncol = 3) +
  labs(x = "Time", y  = "Value", color = "Cue") +
  theme_classic2()

ggarrange(si_dyn_top.pl, ci_dyn_top.pl, labels = c("A", "B"), ncol = 1)
ggsave(here("figures/plos-bio/top-sici_dyn.png"))

# specifically look at fitness accumulation
## single infection all range
si_tauc.pl1 <- si_dyn.df %>% rbind(cbind(si_t.df, id = "time")) %>% 
  filter(id %in% si_top4 & variable == "tau_cum" & time < 20) %>% 
  ggplot() +
  geom_line(aes(x = time, y = value, color = id)) +
  labs(x = "Time (days)", y = "Fitness", color = "Cue") +
  theme_classic()
## single infection with limited range to show difference
si_tauc.pl2 <- si_dyn.df %>% rbind(cbind(si_t.df, id = "time")) %>% 
  filter(id %in% si_top4 & variable == "tau_cum" & time < 20) %>% 
  ggplot() +
  geom_line(aes(x = time, y = value, color = id)) +
  xlim(16,20) +
...
## co-infection with all arange
ci_tauc.pl1 <- ci_dyn.df %>% rbind(cbind(ci_t.df, label = "time")) %>% 
  filter(label %in% ci_top4 & variable == "tau_cum1" & time < 20) %>% 
  ggplot() +
  geom_line(aes(x = time, y = value, color = label)) +
  labs(x = "Time (days)", y = "Fitness", color = "Cue") +
  theme_classic()

## co-infection limited range
ci_tauc.pl2 <- ci_dyn.df %>% rbind(cbind(ci_t.df, label = "time")) %>% 
  filter(label %in% ci_top4 & variable == "tau_cum1" & time < 20) %>% 
  ggplot() +
  geom_line(aes(x = time, y = value, color = label)) +
  xlim(12,20) +
...
## plot together
ggarrange(si_tauc.pl1, si_tauc.pl2, ci_tauc.pl1, ci_tauc.pl2, labels = c("A", "", "B", ""))
ggsave(here("figures/plos-bio/transmission-accumulation.png"), height = 4)
```

#===========================================================#
plotting cumulative transmission potential of si and ci
#===========================================================#
# process datasets
```{r}
# join with time-bsaed optimization and filter only tau (transmission potential)
si_tau.df <- si_dyn.df %>% 
  rbind(cbind(si_t.df, id = "time")) %>% 
  left_join(select(si_fitness.df, id, fitness = value) %>% rbind(data.frame(id = "time", fitness = 9.787899)), by  = "id") %>% 
  filter(variable == "tau" & time < 20) %>% 
  left_join(select(ez_label, ez_label_si, id_si), by = c("id" = "id_si"))

# do the same to ci
ci_tau.df <-ci_dyn.df %>% 
  rbind(cbind(ci_t.df, label = "time")) %>% 
  left_join(select(ci_fitness.df, label, fitness = value) %>% rbind(data.frame(label = "time", fitness = 2.311841)), by  = "label") %>% 
  filter(variable == "tau1" & time < 20) %>% 
  left_join(select(ez_label, ez_label, label_ci), by = c("label" = "label_ci"))

# replace NA in ci with 0 (transmission potential)
ci_tau.df[is.na(ci_tau.df$value), "value"] <- 0
```

# plot tau across time series and ordered based on decreasing fitness
```{r}
# single infection transmission potential
si_tau.pl <- ggplot() +
  geom_raster(data = si_tau.df, aes(x = time, y = fct_reorder(ez_label_si, fitness), fill = value)) +
  scale_fill_viridis_c() +
  labs(x = "Time (days)", y = "Cue", fill = "Fitness") +
  theme_classic() +
  theme(legend.position = "bottom")

ci_tau.pl <- ggplot() +
  geom_raster(data = ci_tau.df, aes(x = time, y = fct_reorder(ez_label, fitness), fill = value)) +
  scale_fill_viridis_c(option = "magma") +
  labs(x = "Time (days)", y = "Cue", fill = "Fitness") +
  theme_classic()+
  theme(legend.position = "bottom")

ggarrange(si_tau.pl, ci_tau.pl, labels = c("A", "B"), ncol = 2)
ggsave(here("figures/plos-bio/si_ci_tau.png"), width = 10, height = 5.5)
```
...
dual_fitness.df <- dual_fitness.df %>% 
  mutate(temp_label = gsub("log", "log10", label),
         ...
         label_final = paste(temp_label, "&", temp_label_b)) 
...
dual_si_fitness.df$dual_label <- as.factor(dual_si_fitness.df$dual_label)
dual_si_fitness.df
```
...
dual_si_fitness.pl <- ggplot() +
  ...
  geom_text(aes(x=9.883602, label="\nTime-based fitness (df=9)", y = "G & I log10"), angle=90) +
  #xlim(8.35, 10) +
  theme_bw() +
  theme(legend.position = "top")
```
...
ggsave(here("figures/plos-bio/dual_cue.tiff"), units = "px", width = 2250, height = 1400, scale = 1.3, dpi=300,  bg = "white")
```
...
# keep only cr and make labels for plotting. 
...
dual_var.ls <- c(unique(dual.cr$variable)[!unique(dual.cr$variable) %in% c("tau", "tau_cum", "cr", "cr_t", "ID", "N", "W", "M", "Mg")], "I_Ig")
...
## replace NA with 0
dual_cr.f[is.na(dual_cr.f)] <- 0
## get I+Ig
dual_cr.f <- dual_cr.f %>% mutate(I_Ig = I+Ig)
dual_cr.f
```
...
# save all 4 plates
ggarrange(plotlist = dual_curve.pl, ncol = 2, nrow = 5, align = "hv")
ggsave(here("figures/plos-bio/dual_curve1.tiff"), units = "px", width = 1000, height = 1200, scale = 1.8, dpi=300,  bg = "white")

ggarrange(plotlist = dual_curve_xlog.pl, ncol = 2, nrow = 5, align = "hv")
ggsave(here("figures/plos-bio/dual_curve2.tiff"), units = "px", width = 1000, height = 1200, scale = 1.8, dpi=300,  bg = "white")

ggarrange(plotlist = dual_curve_ylog.pl, ncol = 2, nrow = 5, align = "hv")
ggsave(here("figures/plos-bio/dual_curve3.tiff"), units = "px", width = 1000, height =1200, scale = 1.8, dpi=300,  bg = "white")

ggarrange(plotlist = dual_curve_log.pl, ncol = 2, nrow = 5, align = "hv")
ggsave(here("figures/plos-bio/dual_curve4.tiff"), units = "px", width = 1000, height = 1200, scale = 1.8, dpi=300,  bg = "white")
```
...
#=============================#
# spline flexibility and solution (4 parameters)
#=============================#
# nested function to generate different iterations of each parameter values
```{r}
spline_par <- function(
  par.df, # range of parameters we want to test
  cue_range)
  { 
  #---------- creating a dummy spline model ----------#
      ## create dummy model
  ...
  #---------- function to get spline for a given parameter value ----------#
      get_spline <- function(par){
          ## assign coefficient to be optimized to the dummy conversion rate function
          dummy_cr.mod$coefficients <- par
          ## get cr
          cr <- exp(-exp(predict(dummy_cr.mod, newdata = data.frame(cue_range))))
          return(cr)
      }
  ...
  #---------- creating a list of parameters to test ----------#
      ...
      changed_par <- mapply(function(x, y){
        ...
        par_trans <- rep(-0.3665129,4) # number needed to start cr = 0.5
  ...
  #---------- get cr from the list of parameters ----------#
      spline.ls <- lapply(par.ls, function(x){
        cbind.data.frame(cr = get_spline(par = x),
                         cue_range = cue_range)
      })
  
      ## attach parameter id to dataframe
      spline_id.ls <- mapply(function(x, y){
        x$id <- y
        return(x)},
        x = spline.ls, y = par.df$id, SIMPLIFY = F)
      
  # Combine results
  res <- do.call(rbind, spline_id.ls)
  return(res)
}
```

# getting cr 
Here, we ran the function to get the cr across an arbitrary cue range (1->100) for a list of parameters.
```{r}
# creating the par.df
par.df4 <- rbind.data.frame(
  data.frame(n = 1, par = seq(-10,10,1)),
  data.frame(n = 2, par = seq(-500,500,1)),
  data.frame(n = 3, par = seq(-500,500,1)),
  data.frame(n = 4, par = seq(-500,500,1))) %>% mutate(id = paste0(n, "_", par))

# run function
exp_spline <- spline_par(par.df = par.df4, cue_range = seq(0,100,1))

# process data. add coefficient name and get parameter values
exp_spline2 <- exp_spline %>% 
  mutate(n = paste("Coefficient", gsub("_.*", "", id)),
         par = gsub("*._", "", id)) %>% 
  distinct(n, par, cue_range, .keep_all = T)

exp_spline2$par <- as.numeric(exp_spline2$par)

# for a given cue value (i.e., 10), we can track how it changes as a function of parameter alterations 
exp_spline2.p <- exp_spline2 %>% 
  group_by(cue_range, n) %>% # we will first group so that we are working with each cue value when a given coefficient is changing
  arrange(par, .by_group = T) %>% # arrange so that cr are arranged at increasing parameter values within each group
  mutate(cr_diff = cr-lag(cr)) # calculate how cr is changing as we change parameters

# plot
spline_flex_4.plt <- ggplot() +
  geom_point(data = exp_spline2.p, aes(x = par, y = cr_diff), size = 0.1) +
  facet_wrap(~n, scales = "free") +
  labs(x = "Parameter values", y = "Change in cr") +
  theme_classic()

ggplot() +
  geom_point(data = exp_spline2.p, aes(x = par, y = cr), size = 0.1) +
  stat_smooth(data = exp_spline2.p, aes(x = par, y = cr)) +
  facet_wrap(~n, scales = "free") +
  labs(x = "Parameter values", y = "Change in cr") +
  theme_classic()
```
...
#=============================#
# spline flexibility and solution (9 parameter)
#=============================#
# nested function to generate different iterations of each parameter values
```{r}
spline_par_9 <- function(
  cue_range,
  cue_range_b,
  par.df) # cue_range optimized over
  { 
  #---------- creating a dummy spline model ----------#
    ## assign conversion rate grid
  ...
  #---------- function that calculates cr based on given parameters ----------#
    get_spline <- function(par){
      # assign coefficient to be optimized to the dummy conversion rate function. Returns conversion rate 
      dummy_cr.mod$coefficients <- par
      pred_cr <- exp(-exp(mgcv::predict.gam(dummy_cr.mod, 
                                     newdata = cr_grid)))
      return(pred_cr)
    }
    
  #---------- get a list of parameters to test ----------#
    ## substitute each value within the 9 parameters with par_seq (generate all iterations)
    changed_par <- mapply(function(x, y){
      ## define original parameter
      par_trans <- rep(-0.3665129,9) # this gives us approximately cr = 0.5 across most ranges!
      ## change according to df
      par_trans[[x]] <- y
      return(par_trans)
    }, x = par.df$n, y = par.df$par)
  ...
  #---------- get cr for the list of parameters ----------#
      spline.ls <- lapply(par.ls, function(x){
        cbind.data.frame(
          cr_grid,
          cr = get_spline(par = x))
      ...
      ## add parameters id to dataframe
      spline_id.ls <- mapply(function(x, y){
        y$id <- x
        return(y)
        },
        y = spline.ls, x = par.df$id, SIMPLIFY = F)
...
# run function
```{r}
# list of parameter ranges we want to test out
par.df9 <- rbind.data.frame(
  data.frame(n = 1, par = seq(-10,10,length.out = 100)),
  data.frame(n = 2, par = seq(-500,500,length.out = 100)),
  data.frame(n = 3, par = seq(-1000,1000,length.out = 100)),
  data.frame(n = 4, par = seq(-1000,1000,length.out = 100)),
  data.frame(n = 5, par = seq(-250,250,length.out = 100)),
  data.frame(n = 6, par = seq(-500,500,length.out = 100)),
  data.frame(n = 7, par = seq(-1000,1000,length.out = 100)),
  data.frame(n = 8, par = seq(-500,500,length.out = 100)),
  data.frame(n = 9, par = seq(-250,250,length.out = 100))
) %>% mutate(id = paste0(n, "_", par))


# get dataframe
exp_spline_9 <- spline_par_9(cue_range = seq(0, 10, by = 1),
                         cue_range_b = seq(0, 10, by = 1),
                         par.df = par.df9)


# process data. add coefficient name and get parameter values
exp_spline_9_2 <- exp_spline_9 %>% 
  mutate(n = paste("Coefficient", gsub("_.*", "", id)),
         par = gsub("*._", "", id)) %>% 
  distinct(n, par, cue_range, cue_range_b, .keep_all = T)

exp_spline_9_2$par <- as.numeric(exp_spline_9_2$par)

# calculate how a single cr changes at a given coordinate as a function of parameter changes
exp_spline_9_2.p <- exp_spline_9_2 %>% 
  group_by(cue_range,cue_range_b, n) %>% # we will first group so that we are working with each cue value when a given coefficient is changing
  arrange(par, .by_group = T) %>% # arrange so that cr are arranged at increasing parameter values within each group
  mutate(cr_diff = cr-lag(cr)) 

# plot
spline_flex_9.plt <- ggplot() +
  geom_point(data = exp_spline_9_2.p, aes(x = par, y = cr_diff), size = 0.1) +
  facet_wrap(~n, scales = "free") +
  labs(x = "Parameter values", y = "Change in cr") +
  theme_classic()

# save
ggarrange(spline_flex_4.plt, spline_flex_9.plt, ncol = 2, labels = c("A", "B"), widths = c(0.7, 1))
ggsave(here("figures/plos-bio/spline_space.tiff"), units = "px", width = 2500, height = 1200, scale = 1.2, dpi=300, bg = "white")
