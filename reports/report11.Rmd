...
res <- optimParallel(par = rep(0.5,4),# new parameter search space
...
# trouble shooting Monte Carlo simulation
```{r}
...
# second iteration of MC. using roptim to avoid double parallelzing
```{r}
monte_carlo2 <- function(i){
...
## start optimization
res <- optim(par = rep(0.5,4),# new parameter search space
                 fn = chabaudi_si_lag_sto2, 
             method = "BFGS",
              control = list(trace = 6, fnscale = -1),
                 immunity = "tsukushi",
...
# trying with mclapply parallelization. Very slow!
```{r}
# does not work with optim parallel
cl <- makeCluster(detectCores()); setDefaultCluster(cl = cl)
mc_out <- mclapply(1:2, monte_carlo)
stopCluster(cl)
...
registerDoSNOW(cl)
cores <- detectCores()
test <- mclapply(1:2, FUN = clusterCall(cl, monte_carlo()), mc.cores = cores)
stopCluster(cl)

# trying cluster apply
...
# Trouble shooting reproducibility
We want our simulations to be a) reproducible and b) for each monte carlo simulation to be independent from one another. Trying the do RNG package
```{r}
library(doParallel)
library(doRNG)
...
registerDoRNG(137)
test <- foreach(i = 1:3, 
                .packages = c("doParallel", "doRNG", "optimParallel", "truncnorm", "deSolve", "splines2", "stringr", "dplyr", "tidyr"),
                .combine=rbind) %dorng% {
                  monte_carlo()
return(res)}
stopCluster(cl)
