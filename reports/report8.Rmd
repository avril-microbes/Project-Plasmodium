...
# get timing of growth, parasetemia and decline
```{r}
# get timing of growth, parasetemia, and decline
ci_dynamics %>% 
...
ci_dynamics %>% 
  ...
  dplyr::group_by(time) %>% 
  dplyr::summarise(sum_I = sum(value)) %>% 
  dplyr::arrange(desc(sum_I)) # 8.581, 725620.6	
...
mean(decline_I$sum_I) # 174519.9

# signal to noise ratio (mean/sd running average with 0.01 days)
ci_I <- ci_dynamics %>% 
  dplyr::filter(variable_alt == "I" | variable_alt == "Ig") %>% 
  dplyr::group_by(time) %>% 
  dplyr::summarise(sum_I = sum(value))
