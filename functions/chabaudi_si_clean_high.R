#-----------------------#
# Single infection model of Plasmodium chabaudi (adopted from 2022-06-22 version of chabaudi_si_clean)
# with higher degree of freedom for allowing higher spline flexibility (k = 8)
# Avril Wang
# Last edited 2022-07-05
# dual cue cable
#-----------------------#

chabaudi_si_clean_high <- function(
  ...
  k = 8 # sets the degree of freedom of spline
){
  ...
  if(cue_b == "none"){
    ...
    dummy_cr.mod <- lm(dummy_y.vals ~ splines2::bSpline(x = cue_range, degree = k))
