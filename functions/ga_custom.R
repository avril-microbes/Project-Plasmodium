...
## Custom edits to simulate malaria
               ...
               seed = NULL) 
{
  ...
  # Fitness will contain mean fitness for every individual
  Fitness <- rep(NA, popSize)
  # Create fitness dataframe
  Fitness_df <- data.frame(matrix(ncol = 3, nrow = length(tournaments)))
  colnames(Fitness_df) <- c("par1","par2","value")
  Fitness_df$par1 <- c(sapply(tournaments, `[[`, 1))
  Fitness_df$par2 <- c(sapply(tournaments, `[[`, 2))
  ...
  for(iter in seq_len(maxiter))
  {
    ...
    {Fitness_df$value <- foreach(i. = seq_len(nrow(Fitness_df)), .combine = "c", .inorder = TRUE) %DO%
      do.call(fitness, c(list(Pop[Fitness_df$par1[i.],], Pop[Fitness_df$par2[i.],]), callArgs)) 
    }
    
    # process value to par1 fitness and par2 fitness
  ## separate the two parameter fitness values
    Fitness_df2 <- Fitness_df
    Fitness_df2$par1_fit <- c(sapply(Fitness_df$value, `[[`, 1))
    Fitness_df2$par2_fit <- c(sapply(Fitness_df$value, `[[`, 2))
    
    ## get mean parameter fitness for each population
    Fitness_df2_1 <- Fitness_df2 %>% 
      dplyr::select(par = par1, par_fit = par1_fit)
    Fitness_df2_2 <- Fitness_df2 %>% 
      dplyr::select(par = par2, par_fit = par2_fit)
    
    Fitness_df3 <- rbind(Fitness_df2_1, Fitness_df2_2)
    
    Fitness_df4 <- Fitness_df3 %>% 
      dplyr::group_by(par) %>% 
      dplyr::summarise(mean_fit = mean(par_fit))
    
    ## assign to Fitness 
    Fitness <- Fitness_df4$mean_fit
