...
chabaudi_ci_opt_lag2 <- function(parameters_cr_1,
                                parameters_cr_2,
                                immunity,
                                parameters,
                                time_range,
  ...
  if(immunity == "ni" || immunity == "i"){
    ...
    state <- c(R = parameters[["R1"]],
               ...
               W = 0,
               A = 0,
               BI = 0, # extent of asexual burst of all strains
               BIg = 0,
               #BI1 = 0, #extent of asexual burst of strain1
               #BIg = 0,
               #BIg1 = 0,
               BIt = 0 # total asexual and sexual burst intensity
               ) # targeted RBC removal
  }
  ...
  chabaudi_ci_model_lag <- function(t, state, parameters) {
    ...
    pulseBeta_1 <- pulseBeta_fun(I0*ratio, sp, t-delay) # only strain 1 can experience delay
    ...
    ### Survival of infected asexual RBC afte alpha 
    ...
    if(t<=alpha+delay){
      ...
      dBI <- mu*(I1+I2)+(-log(1-N)-log(1-W)-log(1-A))*(I1+I2)+(pulseBeta_1*S_1)+(pulseBeta_2*S_2)
      dBIt <- mu*(I1+I2)+(-log(1-N)-log(1-W)-log(1-A))*(I1+I2)+(pulseBeta_1*S_1)+(pulseBeta_2*S_2)
    }
    ...
    if(t<=alpha+alphag+delay && t>alpha+delay){
      dG1 <- 0
      dIg1 <- dIg1_nolag
      dBIg <- mu*(Ig1+Ig2)+(-log(1-N)-log(1-W)-log(1-A))*(Ig1+Ig2)
      dBIt <- mu*(Ig1+Ig2)+(-log(1-N)-log(1-W)-log(1-A))*(Ig1+Ig2)+mu*(I1+I2)+(-log(1-N)-log(1-W)-log(1-A))*(I1+I2)+(p*lag_a_1[1]*lag_a_1[2]*S_1)+(p*lag_a_2[1]*lag_a_2[3]*S_2)
    }
    ...
    if(t>alpha){
      ...
      ## tracking only
      dBI <- mu*(I1+I2)+(-log(1-N)-log(1-W)-log(1-A))*(I1+I2)+(p*lag_a_1[1]*lag_a_1[2]*S_1)+(p*lag_a_2[1]*lag_a_2[3]*S_2)
    ...
    if(t>alpha+alphag+delay){
      ...
      dBIg <- mu*(Ig1+Ig2)+(-log(1-N)-log(1-W)-log(1-A))*(Ig1+Ig2)+(p*lag_b_1[1]*lag_b_1[4]*Sg_1)+(p*lag_b_2[1]*lag_b_2[5]*Sg_2)
      dBIt <- mu*(Ig1+Ig2)+(-log(1-N)-log(1-W)-log(1-A))*(Ig1+Ig2)+(p*lag_b_1[1]*lag_b_1[4]*Sg_1)+(p*lag_b_2[1]*lag_b_2[5]*Sg_2)+mu*(I1+I2)+(-log(1-N)-log(1-W)-log(1-A))*(I1+I2)+(p*lag_a_1[1]*lag_a_1[2]*S_1)+(p*lag_a_2[1]*lag_a_2[3]*S_2)
    }
    
    
    ## Return the states. Must be in the same order as states!
    if (immunity == "ni" || immunity == "i") {return(list(c(dR, dM1, dM2, dMg1, dMg2, dID, dS1, dS2, dI1, dI2, dIg1, dIg2, dG1, dG2, dA)))}
    
    if (immunity == "kochin") {return(list(c(dR, dM1, dM2, dMg1, dMg2, dID, dS1, dS2, dI1, dI2, dIg1, dIg2, dG1, dG2, dE, dA)))}
    
    if (immunity == "tsukushi") {return(list(c(dR, dM1, dM2, dMg1, dMg2, dID, dS1, dS2, dI1, dI2, dIg1, dIg2, dG1, dG2, dN, dW, dA, dBI, dBIg, dBIt)))}
  }
  ...
  if(dyn == FALSE && diff == TRUE){return(tau1.sum-tau2.sum)}
  if(dyn == FALSE && diff == FALSE){return(tau1.sum)}
