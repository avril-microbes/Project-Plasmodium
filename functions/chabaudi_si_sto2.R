...
# based on priors of Miller et al 2010
# Avril Wang
# Last edited 2022-01-22
#-----------------------#


chabaudi_si_lag_sto2 <- function(
  ...
  chabaudi_si_dyn <- function(t, state, parameters){
    ...
    if(t<=alpha+delay){
      ...
      dM <- dM_nolag+(burst_rand[t+1]*pulseBeta*S) # asexual merozoites are produced when asexual iRBC burst
    ...
    if(t>alpha+delay){
      ...
      dM <- dM_nolag+(burst_rand[t+1]*(1-cr)*p*lag1[1]*lag1[2]*S) # production of asexual merozoite from asexual iRBC burst
      dMg <- dMg_nolag+(burst_rand[t+1]*cr*p*lag1[1]*lag1[2]*S) # production of sexual merozoite from asexual iRBC burst
    }
