...
# the resident and mutant parameter. L-BFGS-B is used as optimization algorithm.
  ...
  repeat{
    ...
    if(index == 1){
      ...
      if(fitness < 0 && conv != 0){
        ...
        if(index == 2){residence_par <- parameters_cr} # special case. If first round of optimization fails, go back to initial parametr set
        mutant_par <- par
    ...
    if(conv == 0 && fitness < 0 && lower_repeat == TRUE && fitness > 0 && fitness < limit){
      print("Strain 1 was previously less than strain 2. Now it is fitter and the difference is minute.")
      break
      }
